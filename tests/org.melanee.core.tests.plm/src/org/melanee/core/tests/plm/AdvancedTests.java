...
public class AdvancedTests {
  ...
  public void testAttributeEnumerationDataType() {
    ...
    l.getContent().add(c);
    assertTrue("Result of att1.getDatatype() must be Boolean", att1.getDatatype() == "Boolean");
    ...
    assertTrue("Result of att1.isEnumeration() must be false", att1.isEnumeration() == false);

    att1.setDatatype("enum1");
    assertTrue("Result of att1.isEnumeration() must be true", att1.isEnumeration() == true);
    assertTrue("Result of att1.getEnumeration() must be enum1", att1.getEnumeration() == enum1);
    assertTrue("Result of att1.getLiterals() must contain test1+2+3+4",
        att1.getLiterals().contains("test1") && att1.getLiterals().contains("test2")
            && att1.getLiterals().contains("test3") && att1.getLiterals().contains("test4"));
  ...
  public void testConfigureClabject() {
    ...
    // method under test
    // calls createFeature(Feature template, Clabject templateParent, Clabject
    // newParent, boolean addNewElementToParent)
    // which creates wrong durabilities
    PLMFactory.eINSTANCE.configureClabject(c, newClabject, null);

    assertTrue("The potency of NewClabject should be 0 but is " + newClabject.getPotency(),
        newClabject.getPotency() == 0);
    assertTrue(
        "The durability of the attribute should be 0 but is "
            + newClabject.getFeature().get(0).getDurability(),
        newClabject.getFeature().get(0).getDurability() == 0);
    assertTrue(
        "The mutability of the attribute should be 0 but is"
            + ((Attribute) newClabject.getFeature().get(0)).getMutability(),
        ((Attribute) newClabject.getFeature().get(0)).getMutability() == 0);
  ...
  public void testCreateFromTemplate() {
    ...
    Clabject result1 = PLMFactory.eINSTANCE.createClabjectFromTemplate(template,
    ...
    assertTrue("If potency is zero or parent is null then result should be null",
