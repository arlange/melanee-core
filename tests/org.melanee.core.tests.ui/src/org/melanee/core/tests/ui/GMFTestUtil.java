...
public class GMFTestUtil {
  ...
  /**
   * Executes a PopupBarTool with the specified tool tip. The button is no invoked
   * via the UI but via calls to PopupBarEditPolicy.
   * 
   * @param gefEP
   *          The editpart to execute the button for
   * @param tooltipText
   *          Tool tip of the button
   */
  public static void executeSelectionToolBarToolWithTooltip(final IGraphicalEditPart gefEP,
      String tooltipText) {
    BaseToolbarSelectionEditPolicy pol = (BaseToolbarSelectionEditPolicy) gefEP
        .getEditPolicy(EditPolicy.SELECTION_FEEDBACK_ROLE);

    try {
      Class popupBarEditPolicy = Class.forName(
          "org.melanee.core.modeleditor.custom.selectioneditpolicies.BaseToolbarSelectionEditPolicy");

      // The baloon needs to initialized otherwise a NullPointerException will happen
      // later
      ...
      Method getPopupBarDescriptors = popupBarEditPolicy.getDeclaredMethod("getPopupBarDescriptors",
          null);
      ...
      Class popupBarDescriptor = Class.forName(
          "org.melanee.core.modeleditor.custom.selectioneditpolicies.ToolBarItemDescriptor");
  ...
  public static void connect(String sourceLabel, String targetLabel, SWTBotGefEditor editor) {
    ...
    final IGraphicalEditPart target = ((IGraphicalEditPart) editor.getEditPart(targetLabel).part())
    ...
    UIThreadRunnable.syncExec(new VoidResult() {
      ...
      public void run() {
        ...
        final PaletteViewer palette = ((EditDomain) target.getDiagramEditDomain())
            .getPaletteViewer();
        ...
        NodeToolEntry entry = new NodeToolEntry("ConnectionEnd", "Creates a new ConnectionEnd",
            types);
   ...
   * 
   * @param name
   *          the name of the tab to click on.
   */
  public static void selectTabbedPropertiesPage(final String name, final SWTBot bot) {
    UIThreadRunnable.syncExec(new VoidResult() {
      ...
      @Override
      public void run() {
        List<TabbedPropertyList> items = bot.getFinder()
        ...
        for (TabbedPropertyList item : items) {
          for (Control t : item.getTabList()) {
            if (t instanceof ListElement && name.equals(((ListElement) t).getTabItem().getText())) {
