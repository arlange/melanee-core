...
/**
 * This class is the SWTUtils class extended by the click feature copied from
...
public abstract class SWTUtilsEx extends SWTUtils {
  ...
  /**
   * Sends a non-blocking notification of the specified type to the
   * {@link #widget}.
   * 
   * @param eventType
   *          the type of event.
    ...
    asyncExec(new VoidResult() {
      public void run() {
        if ((widget == null) || widget.isDisposed()) {
          log.trace(MessageFormat.format("Not notifying {0} is null or has been disposed", //$NON-NLS-1$
              widget.toString()));
          return;
        }
   ...
   * 
   * @return an event that encapsulates {@link #widget} and {@link #display}.
   *         Subclasses may override to set other event properties.
   */
