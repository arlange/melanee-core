...
public interface IEmendationService {
	...
	/**
	 * Retruns the ImpactAnalyzer which is shipped with
	 * this emendation service.
	 * 
	...
	/**
	 * Creates an emendation command which supports the move of an 
	 * attribute from one Clabject to an other Clabject
	 * 
	 * @param source Clabject where attribute is moved from
	 * @param target Clabject where attribute is moved to
	 * @param movedFeature Attribute which is moved
	 * @return Complete emendation command for the move operation; 
	 * 				null if the operation is cancelled or no changes are required
	 */
	public Command handleMoveAttributeOperation(Clabject source, Clabject target, Feature movedFeature);
	
	/**
	 * Creates an emendation command which handles the removal of 
	 * an attribute from a Clabject
	 * 
	 * @param changeOrigin Clabject from which attribute is deleted
	 * @param featureToDelete Attribute which is deleted
	 * @return Complete emendation command for the delete operation;
	 * 			null if the operation is cancelled or no changes are required
	 */
	public Command handleRemoveAttributeOperation(Clabject changeOrigin, Feature featureToDelete);
	
	/**
	 * Creates an emenadation command which handles the addition
	 * of an attribute to a Clabject.
	 * 
	 * @param changeOrigin Clabject to which the attribute is added
	 * @param addedFeature Attribute which is added to Clabject
	 * @return Complete emendation command for the add operation;
	 * 			null if the operation is cancelled or no changes are required
	 */
	public Command handleAddAttributeOperation(Clabject changeOrigin, Feature addedFeature);
	
	/**
	 * Creates an emendation command which handles the emendation of
	 * a trait (e.g. durability, name) to a feature
	 * 
	 * @param changeOrigin Feature which owns the changed trait
	 * @param traitToChange Trait which is changed
	 * @param oldValue Old trait value
	 * @param newValue New trait value
	 * @return Complete emendation command for the change operation;
	 * 			null if the operation is cancelled or no changes are required
	 */
	public Command handleChangeFeatureTraitOperation(Feature changeOrigin, EStructuralFeature traitToChange, String oldValue, String newValue);
	
	/**
	 * Creates an emendation command which handles the emendation of
	 * a trait (e.g. potency) of a Clabject
	 * 
	 * @param changeOrigin Clabject which owns the changed trait
	 * @param traitToChange Trait which is changed
	 * @param oldValue Old trait value
	 * @param newValue New trait value
	 * @return Complete emendation command which handles the change operation;
	 * 			null if the operation is cancelled or no changes are required
	 */
	public Command handleChangeClabjectTraitOpertation(Clabject changeOrigin, EStructuralFeature traitToChange, String oldValue, String newValue);
	
	/**
	 * Returns the emended clabjects by one command.
	 * 
	 * @param command Command to get list of emended clabjects for
