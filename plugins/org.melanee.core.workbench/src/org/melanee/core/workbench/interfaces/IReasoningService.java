...
public interface IReasoningService {
   ...
   * 
   * @param modelElement
   *          EObject to execute reasoning on
   * 
  ...
  /**
   * Runs a command in verbose mode. Should call run(String commandID, Object[]
   * parameters, boolean silent) with silent = false in standard implementations.
   * 
   * @param commandID
   *          Id defined by {@link IReasoningService}
   * @param parameters
   *          Parameters defined by the ids
   * 
   ...
   *          Parameters defined by the ids
   * @param run
   *          this one command in silent mode, all others will be verbose again
   * 
   * @return true -> operation sucessful; false -> operation failed
   */
