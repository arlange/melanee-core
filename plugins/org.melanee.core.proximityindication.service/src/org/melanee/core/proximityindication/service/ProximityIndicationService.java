...
public class ProximityIndicationService implements IDesignationService {
  ...
  public String evaluate(Clabject context, String query) {
    return createLocationDesignationString(context, query)
        + createLocationDesignationString(context, query) + context.getName()
        + createClassificationDesignationString(context, query);
  }

  /**
   *
   * @param <T>
   *          Either Clabject or Element in case of containment
   * 
   ...
   * 
   * @param c
   *          the clabject to evaluate the proximity string from
   * @param expression
   *          has the format
   *          [inheritanceExpression](containmentExpression){classificationExpression}
   *          leave empty to display; -1 to display whole hierarchy; 0 to display
   *          first only; seperate levels to display by comma
   * 
  ...
  public String run(Clabject c, String expression) {
    ...
    // Put them together
    String result = inheritanceProximityString + containmentProximityString + c.getName()
        + classificationProximityString;
    return result;
  }
  ...
  public String createLocationDesignationString(Clabject c) {
    ...
    String containmentProximityString = createProximityString(c, containmentRequests,
        new IProximityHelper<Element>() {
  ...
  public String createClassificationDesignationString(Clabject c) {
    String classificationProximityString = createProximityString(c, classificationRequests,
        new IProximityHelper<Clabject>() {
  ...
  public String createHeritageDesignationString(Clabject c) {
    ...
    String inheritanceProximityString = createProximityString(c, generlizationRequests,
        new IProximityHelper<Clabject>() {
  ...
  private String createProximityString(Clabject c, LinkedList<Integer> requestQueue,
      IProximityHelper helper) {
    String result = "";
    ...
    // all the possible elements to contribute to the proximity, each entry is one
    // level of them
    ...
    // Loop through all the possible elements on all the levels and see if there is
    // a request for it
    for (List<? extends Element> currentLevelElements : allLevels) {
      ...
      if (currentRequest == allLevels.indexOf(currentLevelElements) || currentRequest == STAR) {
        String temp = "";
        // If there is more than one element in the current level, we have to bracket
        // them
        if (currentLevelElements.size() > 1)
          temp += "(";
        ...
        if (currentRequest != STAR && currentRequest - previousRequest >= 2)
          temp = helper.appendSeperator(temp);
        // Let the helper build the result
        result = helper.appendResult(result, temp);
        // if the queue is not empty, go to the next request
        // the loop will still do all elements even if there are no more requests
        // because a
  ...
  private void parseInput(String expression) {
    ...
    // Parse the input integer queues of the requested levels from the expression
    // string
