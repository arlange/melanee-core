...
public class ModelExplorerDialog extends TitleAreaDialog {
  ...
  private static final Image SHELL_ICON = AbstractUIPlugin
      .imageDescriptorFromPlugin(Activator.PLUGIN_ID, "/icons/explore.gif").createImage();
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    fileListViewer.addSelectionChangedListener(new ISelectionChangedListener() {
      ...
      public void selectionChanged(SelectionChangedEvent event) {
        try {
          ...
          String fileName = ((IStructuredSelection) event.getSelection()).getFirstElement()
              .toString();
    ...
    ((GridData) sourceOntologyTreeViewer.getControl()
        .getLayoutData()).grabExcessHorizontalSpace = true;
    ((GridData) sourceOntologyTreeViewer.getControl()
        .getLayoutData()).grabExcessVerticalSpace = true;
    ...
    sourceOntologyTreeViewer.addFilter(new ViewerFilter() {
      ...
      @Override
      public boolean select(Viewer viewer, Object parentElement, Object element) {
        return element instanceof Clabject || element instanceof Domain
            || element instanceof DeepModel || element instanceof Feature
            || element instanceof ConnectionEnd || element instanceof Inheritance
    ...
    ((GridData) targetOntologyTreeViewer.getControl()
        .getLayoutData()).grabExcessHorizontalSpace = true;
    ((GridData) targetOntologyTreeViewer.getControl()
        .getLayoutData()).grabExcessVerticalSpace = true;
  ...
  protected void okPressed() {
    ...
    EObject target = (EObject) ((IStructuredSelection) targetOntologyTreeViewer.getSelection())
        .getFirstElement();
   ...
   * 
   * @param source2target
   *          mapping from the source to the target model elements
   */
  private void copyLayout(Map<EObject, EObject> source2target) {
    ...
    while (sourceDiagramIterator.hasNext()) {
      ...
      if (sourceNotaionElement instanceof Node
          ...
          && ((((View) sourceNotaionElement).getElement() instanceof Clabject)
              || (((View) sourceNotaionElement).getElement() instanceof Inheritance))
        ...
        while (targetNotationIterator.hasNext()) {
          ...
          if (targetView instanceof Node && ((Node) targetView).getLayoutConstraint() != null
              && ((View) targetView).getElement() == targetEObject) {
            InternalTransactionalEditingDomain targetEditingDomain = (InternalTransactionalEditingDomain) TransactionUtil
                .getEditingDomain(targetView);
            try {
  ...
  /**
   * Creates the command to copy all model elements from the source to the target
   * 
   * @param targetElements
   *          Copied elements for the target
   * @param target
   *          Element to add the targetElements as child to
   * @param source2target
   *          the mapping of source and target elements
   */
  private void copyElementsToTarget(Collection<EObject> targetElements, EObject target,
    ...
    for (EObject o : targetElements) {
      ...
      if (o instanceof Level) {
        setCommand = SetCommand.create(editingDomain, target,
            PLMPackage.eINSTANCE.getLevel_Content(), ((Level) o).getContent());
      }
      // If a clabject is added the clabject and its content is added
      else if (o instanceof Clabject)
        setCommand = AddCommand.create(editingDomain, target,
            PLMPackage.eINSTANCE.getLevel_Content(), o);
   ...
   *          The resource where the mapping is added to
   */
  private Command createLinksBetweenSourceAndTargetModel(Map<EObject, EObject> source2target,
      Resource targetResource) {

    InternalTransactionalEditingDomain domain = (InternalTransactionalEditingDomain) TransactionUtil
        .getEditingDomain(targetDomain);
    ...
    for (RemoteModel model : linkingModel.getRemoteModels())
      if (model.getPath().equals(
          ((IStructuredSelection) fileListViewer.getSelection()).getFirstElement().toString())
    ...
    if (remoteModel == null) {
      ...
      remoteModel.setPath(
          ((IStructuredSelection) fileListViewer.getSelection()).getFirstElement().toString());
      Command addRemoteModelCommand = new AddCommand(domain, linkingModel,
          LinksPackage.eINSTANCE.getLinkingModel_RemoteModels(), remoteModel);
    ...
    while (sourceIterator.hasNext()) {
      ...
      if (!(source instanceof Clabject || source instanceof Inheritance))
        continue;
    ...
    Command addLinksCommand = AddCommand.create(domain, remoteModel,
        LinksPackage.eINSTANCE.getRemoteModel_Links(), linksToAdd);
