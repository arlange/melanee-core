...
/**
 * <!-- begin-user-doc --> A representation of the model object '<em><b>LML
...
public interface LMLVisualizer extends EObject {
  ...
  /**
   * Returns the value of the '<em><b>Dsl Visualizer</b></em>' containment
   * reference list. The list contents are of type
   * {@link org.melanee.core.models.plm.PLM.AbstractDSLVisualizer}. <!--
   * begin-user-doc -->
   * <p>
   * If the meaning of the '<em>Dsl Visualizer</em>' containment reference list
   * isn't clear, there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * 
   * @return the value of the '<em>Dsl Visualizer</em>' containment reference
   *         list.
   ...
   * Returns the default visualization values depending on the containing model
   * element. <!-- end-model-doc -->
   * 
   * @model kind="operation" annotation="http://www.eclipse.org/emf/2002/Ecore/OCL
   *        body='-- Trait-related default values for all meta-model
   *        elements\r\nlet default:OrderedSet(String) =
   *        self.oclAsType(ecore::EObject).eContainer().eClass().eAllAttributes->collect(a
   *        | if a.name <> \'levelIndex\' then a.name.concat(\'= default\') else
   *        a.name.concat(\'= noshow\') endif)->asOrderedSet()\r\nin\r\n-- Non
   *        trait-related default values for Entity\r\nif
   *        (self.getContainingElement().oclIsTypeOf(Entity))
   *        then\r\n\tdefault->append(\'ProximityClassification= 1\')
   *        -- @deprecated - Do not use anymore\r\n\t\t->append(\'text=
   *        derived\')\r\n\t\t->append(\'graph= derived\')\r\n\t\t->append(\'form=
   *        derived\')\r\n\t\t->append(\'table= derived\')\r\n\t\t->append(\'app=
   *        derived\')\r\n\t\t->append(\'DSLRendering= false\') -- @deprecated -
   *        Do not use anymore\r\n\t\t->append(\'ProximityIndication=
   *        name:1\')\r\nelse\r\n\t-- Non trait-related default values for
   *        Connection\r\n\tif
   *        (self.getContainingElement().oclIsTypeOf(Connection))
   *        then\r\n\t\tdefault->append(\'ProximityClassification= 1\')
   *        -- @deprecated - Do not use anymore\r\n\t\t\t->append(\'text=
   *        derived\')\r\n\t\t\t->append(\'graph=
   *        derived\')\r\n\t\t\t->append(\'form=
   ...
   *        (self.getContainingElement().oclIsTypeOf(Inheritance))
   *        then\r\n\t\t\t\tdefault->append(\'collapsed=
   *        false\')\r\n\t\telse\r\n\t\t\t-- Non trait-related default values for
   *        DeepModel\r\n\t\t\tif
   *        (self.getContainingElement().oclIsTypeOf(DeepModel))
   *        then\r\n\t\t\t\tdefault->append(\'showClassificationConnectionEndNames
   *        = false\')\r\n\t\t\t\t->append(\'text=
   *        derived\')\r\n\t\t\t\t->append(\'graph=
   *        derived\')\r\n\t\t\t\t->append(\'form=
   *        derived\')\r\n\t\t\t\t->append(\'table=
   *        derived\')\r\n\t\t\t\t->append(\'app= derived\')\r\n\t\t\t-- Non
   *        trait-related default values for Level\r\n\t\t\telse\r\n\t\t\t\tif
   *        (self.getContainingElement().oclIsTypeOf(Level))
   *        then\r\n\t\t\t\t\tdefault->append(\'text=
   *        derived\')\r\n\t\t\t\t\t->append(\'graph=
   *        derived\')\r\n\t\t\t\t\t->append(\'form=
   *        derived\')\r\n\t\t\t\t\t->append(\'table=
   *        derived\')\r\n\t\t\t\t\t->append(\'app=
   *        derived\')\r\n\t\t\t\telse\r\n\t\t\t\t\tif
   *        (self.getContainingElement().oclIsTypeOf(Package))
   *        then\r\n\t\t\t\t\t\tdefault->append(\'text=
   *        derived\')\r\n\t\t\t\t\t\t->append(\'graph=
   *        derived\')\r\n\t\t\t\t\t\t->append(\'form=
   *        derived\')\r\n\t\t\t\t\t\t->append(\'table=
