...
/**
 * <!-- begin-user-doc --> A representation of the model object '<em><b>Owned
...
public interface OwnedElement extends Element {
   ...
   * returns the model the clabject is contained in, recursing up the containment
   * path if necessary. <!-- end-model-doc -->
   * 
   ...
   *        (parent.oclIsUndefined()) then\r\n\t\tnull\r\n\telse\r\n\t\tif
   *        (parent.oclIsTypeOf(Level))
   *        then\r\n\t\t\tparent.oclAsType(Level)\r\n\t\telse\r\n\t\t\tif
   *        (parent.oclIsKindOf(Clabject))
   *        then\r\n\t\t\t\tparent.oclAsType(Clabject).getLevel()\r\n\t\t\telse\r\n\t\t\t\tif
   *        (parent.oclIsKindOf(Package))
