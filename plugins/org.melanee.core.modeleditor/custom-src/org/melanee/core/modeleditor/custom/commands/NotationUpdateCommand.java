	...
	public void execute() {
		...
		try {
			...
			if (IVisualizationServiceBase.FORMAT_TEXT.equals(format))
				ExtensionPointService.Instance().getActiveTextualVisualizationService().run(host, notation, false);
			else if (IVisualizationServiceBase.FORMAT_TABLE.equals(format))
				ExtensionPointService.Instance().getActiveTableVisualizationService().run(host, notation, false);
			else if (IVisualizationServiceBase.FORMAT_FORM.equals(format))
				ExtensionPointService.Instance().getActiveFormVisualizationService().run(host, notation, false);
			else if (IVisualizationServiceBase.FORMAT_APP.equals(format))
				ExtensionPointService.Instance().getActiveApplicationVisualizationService().run(host, notation, false);
