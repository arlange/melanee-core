 ...
 *
 *         This class is used to collapse/expand DomainConnections based on
 *         information in the model. Collapsed means that the DomainConnection
 *         is rendered as small black rectangle. Collapsed means that the
 *         DomainConnection will rendered as hexagon, its default figure.
 *
 */
public class ToggleNodeAction implements IObjectActionDelegate {
  ...
  public void run(IAction action) {
    ...
    domain.getCommandStack().execute(SetCommand.create(domain, self.getVisualizer().get(0),
        PLMPackage.eINSTANCE.getLMLVisualizer_Attributes(), attributes));

    // We are not toggeling elided nodes
    /*
     * if (self instanceof Clabject && ((Clabject)self).isElided()) return;
     */
     ...
     * ************************************************************ Needs to be
     * fixed ************************************************************
     * for(Element e : self.getVisualizer().getChildren()) { //We are only
     * interested in fields if (e instanceof Field) { if
     * (e.getName().equals("collapsed")) { collapsedField = (Field)e; break; } } }
     */
    ...
    // new SetRequest(collapsedField, PLMPackage.eINSTANCE.getField_Value(),
    // Boolean.toString(!collapsed));
    // SetValueCommand setNewValueCommand = new SetValueCommand(setNewValueRequest);
    // selectedElement.getViewer().getEditDomain().getCommandStack().execute(new
    // ICommandProxy(setNewValueCommand));
  ...
  // (EditPart)plmEditor.getDiagramGraphicalViewer().findEditPartsForElement(EMFCoreUtil.getProxyID(ont),
  // NodeEditPart.class).get(0);
