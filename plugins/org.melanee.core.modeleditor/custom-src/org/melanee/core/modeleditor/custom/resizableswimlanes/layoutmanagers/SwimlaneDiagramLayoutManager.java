 ...
 *
 * Contributors:
 *    Daniel GritznerRalph Gerbig - initial API and implementation and initial documentation
 *******************************************************************************/
...
/**
 * A layout manager responsible for managing the DeepModel's bounds. Both the width and height
 * are set to values which prevent scrollbars from appearing. The width is also increased to at
 * least the canvas' width.
 * 
...
public class SwimlaneDiagramLayoutManager extends FreeFormLayoutEx {
	...
	// margins used to fit the various EditParts' figures properly into their containers
	 ...
	 * @param deepModel the contained DeepModel
	 */
	...
	/**
	 * determines the minimum width across all Levels contained in the DeepModel which prevents
	 * scrollbars from appearing in any Level
	 * 
	 * @return the minimum width
	 */
	private int getMinimumDeepModelWidth() {
		IGraphicalEditPart levelCompartment = SwimlaneDeepModelLayoutManager.getLevelCompartment(deepModel);
		int minWidth = 0;
	 ...
	 * 
	 * @param figure the containing diagram's figure
	 */
	@Override
	public void layout(IFigure figure) {
		super.layout(figure); // invoke default implementation to ensure that all children are rendered properly
		
		// determine the height required so that no scrollbars are necessary yet only small margins are visible
		int height = 0;
		for(Object child : deepModel.getChildren()) {
			if(child instanceof DeepModelDeepModelLevelCompartmentEditPart) {
				height += Math.max(SwimlaneDeepModelLayoutManager.getTotalLevelHeight(deepModel), MINIMUM_LEVEL_COMPARTMENT_HEIGHT);
			} else {
		...
		// are the two lines below left-overs from when deep models were resizable in all directions? they cause problems when zooming out now
		...
		//System.out.println(bounds + " ||| " + diagramBounds + " ||| " + getMinimumDeepModelWidth());
