...
/**
 * EditPolicy which ensures that the container is resized if an entity inside it is moved or
 * resized. 
 * 
 * @author gritzner
 */
public class ContentAwareContainerCompartmentLayoutEditPolicy extends XYLayoutEditPolicy {
	...
	/**
	 * ensures that every new Entity is created no further to the left or to the top than
	 * (0,0)
	 */
	@Override
	 ...
	 * 
	 * @param deltaX The change in width. Negative values are ignored.
	 * @param deltaY The change in height. Negative values are ignored.
	 * @return the Command
	 */
	...
	/**
	 * Checks whether the container has to be resized when fulfilling the move command. Returns
	 * an appropriate CompoundCommand which also resizes the container if necessary.
	 */
	@Override
	protected Command getMoveChildrenCommand(Request _request) {
		...
		paneBounds.height = parent.getFigure().getBounds().height - host.getFigure().getBounds().y - HEIGHT_MARGIN;
	...
	/**
	 * Checks whether the container has to be resized when fulfilling the reize command. Returns
	 * an appropriate CompoundCommand which also resizes the container if necessary.
	 */
	@Override
