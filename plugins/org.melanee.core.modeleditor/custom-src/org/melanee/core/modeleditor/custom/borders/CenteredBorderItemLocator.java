...
/**
 * 
 * Positions a figure at the centered south. If an other position than south is
 * chosen it behaves like the standard BorderItemLocator
 *
 */
public class CenteredBorderItemLocator extends BorderItemLocator {
  ...
  public void setInitialOffset(int x, int y) {
    initialOffset = new Dimension(x, y);
  }

  public void setInitialOffset(Dimension d) {
    initialOffset = d;
  }

  public Dimension getInitialOffset() {
  ...
  protected Point getPreferredLocation(int side, IFigure borderItem) {
    ...
    if (side == PositionConstants.NORTH_WEST) {
      ...
      x = parentFigureX - getSize(borderItem).width + getBorderItemOffset().width;
      y += (parentFigureHeight / 2) - (borderItemSize.height / 2);
  ...
  @Override
  protected void calculateNextNonConflictingPosition(Point currentLocation, int interval,
      int currentSide, IFigure borderItem, Rectangle obstacle) {

    super.calculateNextNonConflictingPosition(currentLocation, interval, currentSide, borderItem,
        obstacle);

    if (currentSide == PositionConstants.SOUTH_EAST)
      super.calculateNextNonConflictingPosition(currentLocation, interval, PositionConstants.SOUTH,
          borderItem, obstacle);
  }
}
