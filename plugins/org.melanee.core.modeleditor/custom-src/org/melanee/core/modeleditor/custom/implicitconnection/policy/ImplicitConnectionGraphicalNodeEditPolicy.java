 ...
 * 
 * <ul>
 * 	<li>Enable advanced link creation (two connectionEnds + connection with one line)</li>
		...
		if (request instanceof CreateUnspecifiedTypeConnectionRequest)
		{
			...
			try {
				...
				if (source instanceof Entity && target instanceof Entity)
					...
					connectionEnds = ExtensionPointService.Instance().getActiveDSLService().getInstantiableConnectionEndsBetween((Connection)source, (Entity)target);
			...
			protected ILabelProvider getLabelProvider() {
				return new LabelProvider(){
					...
					public String getText(Object object) {
						...
						else if (object instanceof MetamodelType && ((MetamodelType)object).getDisplayName().equals("ConnectionEnd")
																 && content.size() > 1
