...
/**
 * EditPolicy ensuring that no entity with a LevelDomainElementsCompartment can
 * be added to or moved to a location which would cause scrollbars to appear. If
 * the new location is too far to the right or too far to the bottom the
 * DeepModel or Level is automatically resized. Also forwards Level creation
 * requests to the LevelCompartment to allow adding new Levels by clicking on
 * existing ones.
 * 
 * @author gritzner
 */
public class SwimlaneLevelDomainElementsCompartmentLayoutEditPolicy extends XYLayoutEditPolicy {

  /**
   * in addition to the default implementation enforces a minimum location of
   * (0,0) to prevent scrollbars
   */
  @Override
  ...
  /**
   * creates a Command to resize the Level containing the
   * LevelDomainElementsCompartment using this EditPolicy
   * 
   * @param deltaY
  ...
  /**
   * In addition to the default implementation a minimum location of (0,0) is
   * enforced. Also the proper DeepModel/Level resize commands are created when
   * moving an entity too far to the right or too far to the bottom.
   */
  @Override
  ...
  /**
   * Returns a CompoundCommand also resizing the Level and/or DeepModel if
   * necessary after executing the resize operation according to the request.
   */
  @Override
  ...
  /**
   * forwards Level creation requests to the LevelCompartment in addtion to the
  ...
  public Command getCommand(Request request) {
    ...
    if (cmd == null && request instanceof CreateUnspecifiedTypeRequest) {
      ...
      if (cutRequest.getElementTypes().contains(PLMElementTypes.Level_3097)) {
        ...
        CreateUnspecifiedTypeRequest createLevelRequest = new CreateUnspecifiedTypeRequest(types,
            prefHint);
