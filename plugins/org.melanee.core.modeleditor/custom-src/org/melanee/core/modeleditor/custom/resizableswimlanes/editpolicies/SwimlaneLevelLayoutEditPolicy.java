...
/**
 * EditPolicy which forwards Level creation request to the LevelCompartment to
 * allow creating new Levels by clicking on exiting ones
 * 
 * @author gritzner
 */
public class SwimlaneLevelLayoutEditPolicy extends LayoutEditPolicy {
  ...
  // /**
  // * forwards Level creation requests to the LevelCompartment in addition to the
  // default implementation
  // */
  // @Override
  // public Command getCommand(Request request) {
  // ((IGraphicalEditPart)getHost()).getFigure().invalidateTree(); // ensure that
  // the LevelDomainElementsCompartment's figure has properly set bounds
  // Command cmd = super.getCommand(request);
  //
  // if(cmd == null && request instanceof CreateUnspecifiedTypeRequest) {
  // CreateUnspecifiedTypeRequest cutRequest =
  // (CreateUnspecifiedTypeRequest)request;
  //
  // if(cutRequest.getElementTypes().contains(PLMElementTypes.Entity_3105)) {
  // EditPart levelDomainElementsCompartment =
  // SwimlaneDeepModelLayoutManager.getLevelDomainElementsCompartment((IGraphicalEditPart)getHost());
  ...
  // CreateUnspecifiedTypeRequest createLevelRequest = new
  // CreateUnspecifiedTypeRequest(types, prefHint);
  //
  // // send new request to the LevelCompartment
  // EditPart levelCompartment = getHost();
  // while(!(levelCompartment instanceof
  // DeepModelDeepModelLevelCompartmentEditPart))
