...
/**
 * A layout manager responsible for managing the Levels within a DeepModel.
 * Appropriate bounds are chosen such that no scrollbars are necessary.
 * 
 * @author gritzner
 */
public class SwimlaneDeepModelLayoutManager extends StackLayout {
   ...
   * 
   * @param level
   *          the Level whose state is being checked
  ...
  /**
   * returns the smallest width delta for a Level which does not cause scrollbars
   * to appear
   * 
   * @param level
   *          the Level
  ...
  /**
   * Returns the height of a level which may be changed soon (e.g. when handling a
   * ChangeBoundsRequest). The returned height accounts for the incoming change
   * and also enforces a minimal height prevent scrollbars.
   * 
   * @param level
   *          the Level
   * @param deltaHeight
   *          how much the Level's height will be changed soon
   * @param skipCollapsedCheck
   *          if true calculate the Level's height as if it is not collapsed
   * @return the Level's new height
   */
  public static int getLevelHeight(IGraphicalEditPart level, int deltaHeight,
      boolean skipCollapsedCheck) {
    IFigure figure = level.getFigure();
    int minHeight = MINIMUM_LEVEL_HEIGHT; // figure.getMinimumSize() seems to return only (0,0)
                                          // after switching to XYLayouts for Levels
    ...
    if (skipCollapsedCheck || !levelIsCollapsed(level)) {
      ...
      // determine height of current EditPart and make sure that the new height
      // respects the minimum height
   ...
   * 
   * @param compartment
   *          the LevelDomainElementsCompartment
   * @param width
   *          its new width
   */
  ...
  /**
   * Sets the bounds of all Levels and their LevelDomainElementsCompartments.
   * Ensures appropriate values are set to prevent scrollbars.
   * 
   * @param levelCompartment
   *          the compartment containing all the Levels
   */
  private void layoutLevels(IGraphicalEditPart levelCompartment) {
    ...
    for (Object child : levelCompartment.getChildren()) {
      ...
      updateDomainElementsCompartment(domainElementsCompartment,
          width - LEVEL_DOMAIN_ELEMENTS_COMPARTMENT_MARGIN);
    }
  }

  /**
   * Sets the bounds of all Levels of a DeepModel. Ensures appropriate values are
   * set to prevent scrollbars.
   * 
   * @param figure
   *          the DeepModel's figure
   */
  @Override
