...
/**
 * 
 * This is the selection edit policy which is installed on all edit parts. It is
 * to replace the pop up bar with a tool bar that appears on selection instead
 * of hovering.
 * 
 * Code is copied from
 ...
 *
 */
public class BaseToolbarSelectionEditPolicy extends SelectionEditPolicy
    implements MouseMotionListener {

  /**
   * This flag is for the future. It is for example a problem to hover over
  ...
  /**
   * The amount of time to wait before hiding the diagram assistant after the user
   * has moved the mouse outside of the editpart.
   */
  ...
  /**
   * Sets the current mouse location. If set to null, this implies that the mouse
   * is outside the host and diagram assistant figure.
   * 
   * @param mouseLocation
  ...
  protected void hideSelection() {
    ...
    // ((IGraphicalEditPart) getHost()).getFigure().removi
    // neMouseMotionListener(this);
  ...
  protected void showSelection() {
    ...
    Point p = new Point(getHostFigure().getBounds().getTopRight().x + 5,
        getHostFigure().getBounds().getTopRight().y);
  ...
  static private int POPUPBAR_DISPLAYATMOUSEHOVERLOCATION = 0x04; /*
                                                                   ...
                                                                   * mouse location used by diagrams
                                                                   * and machine edit parts
                                                                   */
  ...
  /**
   * Populates the popup bar with popup bar descriptors added by suclassing this
   * editpolicy (i.e. <code>fillPopupBarDescriptors</code> and by querying the
   * modeling assistant service for all types supported on the popup bar of this
   * host. For those types added by the modeling assistant service the icons are
   * retrieved using the Icon Service.
   */
  protected void populatePopupBars() {
    ...
    for (Iterator iter = types.iterator(); iter.hasNext();) {
      ...
      if (type instanceof IElementType) {
        addPopupBarDescriptor((IElementType) type,
            IconService.getInstance().getIcon((IElementType) type));
      }
    }
  }

  /**
   * This is the entry point that subclasses can override to fill the popup bar
   * descrioptors if they have customized tools that cannot be done using the type
   * along with the modeling assistant service.
   */
  ...
  protected void addPopupBarDescriptor(IElementType elementType, Image theImage,
    ...
    ToolBarItemDescriptor desc = new ToolBarItemDescriptor(theTip, theImage, elementType,
        theTracker);
   ...
   * @param children
   */
  protected void addPopupBarMenuDescriptor(Image theImage, DragTracker theTracker, String theTip,
      List<ToolBarItemDescriptor> children) {
    myPopupBarDescriptors
        .add(new ToolBarMenuDescriptor(theTip, theImage, null, theTracker, children));
  }
  ...
  /**
   * default method for plugins which passes along the PopupBarTool as the tool to
   * be used.
   * 
   * @param elementType
   * @param theImage
  ...
  private void initPopupBars() {
    ...
    for (Object desc : theList) {
      iTotal += ((ToolBarItemDescriptor) desc).getToolTip().equals(SeparatorPopupBarTool.TOOL_TIP)
          ? SEPARATOR_WIDTH
          : ACTION_WIDTH_HGT;
    }
    ...
    for (Iterator iter = theList.iterator(); iter.hasNext();) {
      ...
      if (theDesc instanceof ToolBarMenuDescriptor)
        b = new DropDownToolbarItem(theDesc.getDragTracker(), theDesc.getIcon(), this,
            ((ToolBarMenuDescriptor) theDesc).getChildren());
      else
        b = new ToolbarItem(theDesc.getDragTracker(), theDesc.getIcon(), this);
      ...
      if (!theDesc.getToolTip().equals(SeparatorPopupBarTool.TOOL_TIP)) {
        ...
        r1.setSize(SEPARATOR_WIDTH, ACTION_WIDTH_HGT - ACTION_MARGIN_RIGHT);
      }
   ...
   *          The reference point which may be used to determine where the diagram
  ...
  /**
   * Uses the balloon location passed in and its size to determine if the balloon
   * will appear outside the viewport. If so, the balloon location will be
   * modified accordingly.
   * 
   * @param balloonLocation
   *          the suggested balloon location passed in and potentially modified
  ...
  private class FocusListener extends EditPartListener.Stub {
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.gef.EditPartListener#selectedStateChanged(org.eclipse.gef.
     * EditPart)
     */
  ...
  /**
   * Gets the current mouse location. This will be null if the mouse is outside
   * the host and diagram assistant figure.
   * 
  ...
  /**
   * Gets the amount of time to wait before hiding the diagram assistant after the
  ...
  private class HideDiagramAssistantJob extends UIJob {
    ...
    /**
     * The diagram assistant is removed when this task is run if the mouse is still
     * outside the shape.
     */
  ...
  protected boolean isDiagramAssistant(Object object) {
    return object instanceof Toolbar || object instanceof ToolbarItem;
  }
  ...
  /**
   * Gets the amount of time to wait before hiding the diagram assistant after it
  ...
  private class ShowDiagramAssistantJob extends UIJob {
    ...
    /**
     * The diagram assistant added when this task is run if the mouse is still at
     * the same spot where it was when the timer was started (i.e. only add the
     * diagram assistant when the user stops moving the mouse).
     */
    public IStatus runInUIThread(IProgressMonitor monitor) {
