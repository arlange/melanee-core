...
/**
 * EditPolicy which ensures that an Entity can never become smaller than is
 * necessary to render all contained entities without scrollbars
 * 
 * @author gritzner
 *
 */
public class ContentAwareContainerEditPolicy extends ConstrainedToolbarLayoutEditPolicy {
  ...
  // Checks whether `editPart' is currently being rendered in a DSL. Returns true
  // if it is.
  private static boolean isDSLrendered(IGraphicalEditPart editPart) {
    ...
    if (editPart instanceof EntityEditPart)
      dslRendering = AspectAwareVisualizationSearchAlgorithm.searchNotationForElement(
          (Element) editPart.resolveSemanticElement(),
          IVisualizationServiceBase.FORMAT_GRAPH) != IVisualizationServiceBase.NOTATION_LML;
  ...
  /**
   * Changes a ChangeBoundsRequest such that it will not change the supplied
   * EditPart in a way that causes scrollbars to appear in its entity content
   * compartment.
   * 
   * @param editPart
   *          the EditPart
   ...
   */
  public static void constrainChangeBoundsRequest(IGraphicalEditPart editPart,
