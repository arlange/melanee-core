		...
		return new Command() {
			...
			public void execute() {
				...
				entry.setId("dsl." + EMFCoreUtil.getProxyID(type)
						+ ModelEditorConstants.INSTANTIATE_WITH_CONTENT);
	...
	private static class UnspecifiedTypeCreationToolEx extends UnspecifiedTypeCreationTool{
		...
		protected Command getCommand() {
			...
			Command synchronizeCommand = new Command("Synchronize Visualizers and Notation model") {
				@Override
				public void execute() {
					org.eclipse.gef.commands.CompoundCommand elementAndViewCompoundCommand = (org.eclipse.gef.commands.CompoundCommand) result.getCommands().get(1);
					SetBoundsCommand sbc = (SetBoundsCommand)((ICommandProxy)elementAndViewCompoundCommand.getCommands().get(1)).getICommand();
					try {
						...
						InternalTransactionalEditingDomain domain = (InternalTransactionalEditingDomain)TransactionUtil.getEditingDomain(rootNode);
						domain.startTransaction(false, java.util.Collections.EMPTY_MAP);
						
						//For the rootNode we set width and height only, position is determined by mouse click
						...
						while(iter.hasNext()){
							...
							if (nextNode.getLayoutConstraint() == null || ! (nextNode.getLayoutConstraint() instanceof Bounds))
								continue;
							
							Bounds nextBounds = (Bounds)nextNode.getLayoutConstraint();
							LMLVisualizer nextVisualizer = ((Element)nextNode.getElement()).getVisualizer().get(0);
