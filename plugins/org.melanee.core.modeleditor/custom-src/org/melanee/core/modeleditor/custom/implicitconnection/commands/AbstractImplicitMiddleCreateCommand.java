...
public abstract class AbstractImplicitMiddleCreateCommand extends EditElementCommand {
  ...
  public AbstractImplicitMiddleCreateCommand(CreateRelationshipRequest request, EObject source,
      EObject target) {
  ...
  protected abstract IElementTypeAwareAdapter createSemanticMiddle(IProgressMonitor monitor,
      IAdaptable info) throws ExecutionException;

  protected abstract IElementTypeAwareAdapter createImplicitTargetLink(IProgressMonitor monitor,
      IAdaptable info) throws ExecutionException;

  protected abstract void setupSemanticConnections(EObject source, IElementTypeAwareAdapter middle,
      EObject target);
  ...
  protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
    ...
    try {
      IElementTypeAwareAdapter semanticMiddle = createSemanticMiddle(monitor,
          new EObjectAdapter(getSourceView().getElement().eContainer()));
      middle = (EObject) semanticMiddle.getAdapter(EObject.class);

      View container = (View) getSourceView().eContainer();
      Node middleNode = getViewService().createNode(semanticMiddle, container,
          semanticMiddle.getSemanticHint(), ViewUtil.APPEND, getPreferencesHint());

      IElementTypeAwareAdapter semanticImplicitLink = createImplicitTargetLink(monitor, info);
      ...
      myCommandsPack.getSetConnectionEndsCommand()
          .setNewSourceAdaptor(new EObjectAdapter(middleNode));
      myCommandsPack.getSetConnectionEndsCommand()
          .setNewTargetAdaptor(new EObjectAdapter(getSourceView()));
      // Set the new anchor
      // Anchor needs to be set to prevent NullPointerException in combination
      // with DefaultSizeNodeFixedAnchors
      myCommandsPack.getSetConnectionAnchorsCommand()
          .setNewSourceTerminal(calculateConnectionAnchor(middleNode, (Node) getSourceView()));

      Edge implicitTargetEdge = (Edge) getViewService().createEdge(semanticImplicitLink,
          container.getDiagram(), semanticImplicitLink.getSemanticHint(), ViewUtil.APPEND,
          getPreferencesHint());
  ...
  protected Dimension getImplicitNodeSize(Node implicitNode) {
    if (implicitNode.getLayoutConstraint() instanceof Size) {
      ...
      if (size.eIsSet(NotationPackage.eINSTANCE.getSize_Width())
        ...
        return new Dimension(positiveOrZero(size.getWidth()), positiveOrZero(size.getHeight()));
      }
    }
  ...
  protected final void invokeDefaultElementTypeConfiguration(
      IElementTypeAwareAdapter semanticAdapter, IProgressMonitor monitor, IAdaptable info)
      throws ExecutionException {
    IElementType elementType = semanticAdapter.getElementType();
    EObject newElement = (EObject) semanticAdapter.getAdapter(EObject.class);
    if (newElement != null) {
  ...
  private static Point getCenterPoint(View view) {
    ...
    if (constraint instanceof Size) {
      ...
      result.translate(positiveOrZero(width) / 2, positiveOrZero(height) / 2);
    }
  ...
  private String calculateConnectionAnchor(Node source, Node target) {
    ...
    if (Math.abs(dy) > Math.abs(dx))
      return dy < 0 ? DefaultSizeNodeFixedAnchors.ANCHOR_SOUTH
          : DefaultSizeNodeFixedAnchors.ANCHOR_NORTH;
    else
      return dx < 0 ? DefaultSizeNodeFixedAnchors.ANCHOR_EAST
          : DefaultSizeNodeFixedAnchors.ANCHOR_WEST;
