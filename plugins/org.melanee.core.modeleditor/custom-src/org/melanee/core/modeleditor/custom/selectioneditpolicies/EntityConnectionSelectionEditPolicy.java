...
/**
 * This edit policy is responsible for making compartments visible when hovering
 * over an model element with a tool. E.g. if an attribute tool is selected
 * while hovering the attributes compartment will become visible.
 *
 */
public class EntityConnectionSelectionEditPolicy extends ExtendableToolbarSelectionEditPolicy {
  ...
  public void showTargetFeedback(Request request) {
    ...
    // Code shall only be executed when a tool is selected which can add a child to
    // entity
    ...
    if (req.getElementTypes().contains(PLMElementTypes.Attribute_3100)
        || req.getElementTypes().contains(PLMElementTypes.Attribute_3106))
      compartments |= EntityEditPart.COMPARTMENT_ATTRIBUTES; // this constant should be the same in
                                                             // Entity2-, Connection- and
                                                             // Connection2EditPart
  ...
  public void eraseTargetFeedback(Request request) {
    ...
    // Search in the stack if this code is called from an element creation via a
    // tool
    ...
    if (!elementCreated) {
      AbstractClabjectEditPart clabjectEditPart = AbstractClabjectEditPart.create(getHost());
      clabjectEditPart.makeCompartmentsVisible(EntityEditPart.COMPARTMENT_AUTOMATIC); // hides empty
                                                                                      // compartments
    }
