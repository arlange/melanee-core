 ...
 *
 */
public class ExtendableToolbarSelectionEditPolicy extends BaseToolbarSelectionEditPolicy {

  /**
   * We override populatePopupBars because we want to replace what is shown. If
   * the defult content is to be extended, fillPopupBarDescritpor needs to be
   * called.
   */
  @Override
  protected void populatePopupBars() {
    ...
    try {
       ...
       ***********************************************************/
      if (!(semanticElement instanceof Inheritance) && !(semanticElement instanceof Method)) {
        ...
        if (ExtensionPointService.Instance().getActiveTextualVisualizationService() != null) {
          PopupToolBarToolWrapper textToolWrapper = createMenuForFormat(
              IVisualizationServiceBase.FORMAT_TEXT);
          wrappers.add(textToolWrapper);
        }

        if (ExtensionPointService.Instance().getActiveTableVisualizationService() != null) {
          PopupToolBarToolWrapper tableToolWrapper = createMenuForFormat(
              IVisualizationServiceBase.FORMAT_TABLE);
          wrappers.add(tableToolWrapper);
        }

        if (ExtensionPointService.Instance().getActiveFormVisualizationService() != null) {
          PopupToolBarToolWrapper formToolWrapper = createMenuForFormat(
              IVisualizationServiceBase.FORMAT_FORM);
          wrappers.add(formToolWrapper);
        }

        if (ExtensionPointService.Instance().getActiveApplicationVisualizationService() != null) {
          PopupToolBarToolWrapper appToolWrapper = createMenuForFormat(
              IVisualizationServiceBase.FORMAT_APP);
       ...
       ***********************************************************/
      if (!(semanticElement instanceof Entity) && !(semanticElement instanceof Level)
          && !(semanticElement instanceof Package) && !(semanticElement instanceof Method)) {
        tool = new TogglePopupBarTool(getHost(), null);

        imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
            "/icons/popup/toggle16.gif");
        image = imageDescriptor.createImage();

        PopupToolBarToolWrapper toolWrapper = new PopupToolBarToolWrapper(image, tool,
            "Expand/Collapse", "connector");
      ...
      if (semanticElement instanceof Method) {
        ...
        imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
            "/icons/popup/parameter_plus16.gif");
        image = imageDescriptor.createImage();
        PopupToolBarToolWrapper toolWrapper = new PopupToolBarToolWrapper(image, tool,
        ...
        imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
            "/icons/popup/parameter_minus16.gif");
        image = imageDescriptor.createImage();
      ...
      if (((IGraphicalEditPart) getHost()).resolveSemanticElement() instanceof Entity
        ...
        // *************************************************
        imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
            "/icons/popup/instantiate16.gif");
        image = imageDescriptor.createImage();
        PopupToolBarMenuWrapper menu = new PopupToolBarMenuWrapper(image, null,
            "Instantiate Model Element", new LinkedList<PopupToolBarToolWrapper>(), "DSL");

        // *************************************************
        // Do a single instantiation
        // *************************************************
        tool = new InstantiatateSinglePopupBarTool(getHost(), null);
        imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
            "/icons/popup/instantiate_single16.gif");
        image = imageDescriptor.createImage();

        PopupToolBarToolWrapper toolWrapper = new PopupToolBarToolWrapper(image, tool,
            "Instantiate without Content", "clabject");
        ...
        // *************************************************
        tool = new InstantiatateWithContentPopupBar(getHost(), null);
        imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
            "/icons/popup/instantiate_content16.gif");
        image = imageDescriptor.createImage();

        toolWrapper = new PopupToolBarToolWrapper(image, tool, "Instantiate with Content",
            "clabject");
      ...
      for (IPopupToolBarProvider provider : ExtensionPointService.Instance()
          .getAllPopUpBarButtonProvider()) {
        // Get the popup bar tool for the model element the tool is built for
        for (PopupToolBarToolWrapper toolWrapper : provider
            .getPopUpToolbarButtonsForDiagramElement(getHost())) {
          // Get all wrappers stored under ID of tool
          List<PopupToolBarToolWrapper> wrappersForGroup = groups2tools
              .get(toolWrapper.getGroupId());
        ...
        // addPopupBarDescriptor(null, (Image)toolWrapper.getImage(),
        // (AbstractPopupBarTool)toolWrapper.getTool(), toolWrapper.getText());
      }
      ...
      for (List<PopupToolBarToolWrapper> wrapperGroup : groups2tools.values()) {
        ...
        for (PopupToolBarToolWrapper toolWrapper : wrapperGroup)
          if (toolWrapper instanceof PopupToolBarMenuWrapper) {
            ...
            for (PopupToolBarToolWrapper childTool : ((PopupToolBarMenuWrapper) toolWrapper)
                .getChildren())
              childTools.add(new ToolBarItemDescriptor(childTool.getText(),
                  (Image) childTool.getImage(), null, (AbstractPopupBarTool) childTool.getTool()));
            addPopupBarMenuDescriptor((Image) toolWrapper.getImage(),
                (AbstractPopupBarTool) toolWrapper.getTool(), toolWrapper.getText(), childTools);
          } else
            addPopupBarDescriptor(null, (Image) toolWrapper.getImage(),
                (AbstractPopupBarTool) toolWrapper.getTool(), toolWrapper.getText());

        // Add an separator between this and the next group
        AbstractPopupBarTool seperatorTool = new SeparatorPopupBarTool(getHost(), null);

        imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
            "/icons/popup/seperator16.gif");
        image = imageDescriptor.createImage();
     ...
     ***********************************************************/
    /*
     * if (((IGraphicalEditPart)getHost()).resolveSemanticElement() instanceof
     * Clabject){ tool = new IntrospectPopupBarTool(getHost(), null);
     * 
     * imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
     * "/icons/introspect16.gif"); image = imageDescriptor.createImage();
     * 
   ...
   * 
   * @param format
   *          supported formats are graph, text, table and form
   * @return
   */
  private PopupToolBarToolWrapper createMenuForFormat(String format) {
    ...
    AbstractPopupBarTool tool = new NotationSelectionPopupBarTool(getHost(), null,
        IVisualizationServiceBase.NOTATION_DERIVED, format);

    ImageDescriptor imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
        "/icons/popup/derived16.gif");
    Image image = imageDescriptor.createImage();
    PopupToolBarToolWrapper toolWrapper = new PopupToolBarToolWrapper(image, tool, "Derived",
        "clabject");
    children.add(toolWrapper);

    // Add GPL notation
    tool = new NotationSelectionPopupBarTool(getHost(), null,
        IVisualizationServiceBase.NOTATION_LML, format);

    imageDescriptor = Activator.imageDescriptorFromPlugin(Activator.PLUGIN_ID,
        "/icons/popup/predefined16.gif");
    image = imageDescriptor.createImage();
    ...
    try {
      Set<AbstractDSLVisualizer> set = (Set<AbstractDSLVisualizer>) OCLHelper.execute(
          ((IGraphicalEditPart) getHost()).resolveSemanticElement(),
          "AbstractDSLVisualizer.allInstances()");
