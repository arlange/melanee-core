...
public abstract class PLMAbstractExpression {
  ...
  protected void setStatus(int severity, String message, Throwable throwable) {
    ...
    if (!this.status.isOK()) {
      PLMDiagramEditorPlugin.getInstance().logError(
          "Expression problem:" + message + "body:" + body(), //$NON-NLS-1$//$NON-NLS-2$
          throwable);
    }
  }
  ...
  public Object evaluate(Object context, Map env) {
    if (context().isInstance(context)) {
      try {
        ...
        PLMDiagramEditorPlugin.getInstance().logError("Expression evaluation failure: " + body(), //$NON-NLS-1$
            e);
      }
    }
    return null;
