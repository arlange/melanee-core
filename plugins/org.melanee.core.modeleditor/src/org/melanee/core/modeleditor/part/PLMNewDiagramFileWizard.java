...
public class PLMNewDiagramFileWizard extends Wizard {
	...
	* @generated
	*/
	public PLMNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot, TransactionalEditingDomain editingDomain) {
		assert domainModelURI != null : "Domain model uri must be specified"; //$NON-NLS-1$
		assert diagramRoot != null : "Doagram root element must be specified"; //$NON-NLS-1$
		assert editingDomain != null : "Editing domain must be specified"; //$NON-NLS-1$

		myFileCreationPage = new WizardNewFileCreationPage(Messages.PLMNewDiagramFileWizard_CreationPageName,
				StructuredSelection.EMPTY);
		myFileCreationPage.setTitle(Messages.PLMNewDiagramFileWizard_CreationPageTitle);
		myFileCreationPage.setDescription(
				NLS.bind(Messages.PLMNewDiagramFileWizard_CreationPageDescription, DomainEditPart.MODEL_ID));
		IPath filePath;
		...
		myFileCreationPage.setFileName(PLMDiagramEditorUtil.getUniqueFileName(filePath, fileName, "lml")); //$NON-NLS-1$

		diagramRootElementSelectionPage = new DiagramRootElementSelectionPage(
				Messages.PLMNewDiagramFileWizard_RootSelectionPageName);
		diagramRootElementSelectionPage.setTitle(Messages.PLMNewDiagramFileWizard_RootSelectionPageTitle);
		diagramRootElementSelectionPage.setDescription(Messages.PLMNewDiagramFileWizard_RootSelectionPageDescription);
	...
	*/
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
				...
				if (diagramVID != DomainEditPart.VISUAL_ID) {
					return CommandResult.newErrorCommandResult(Messages.PLMNewDiagramFileWizard_IncorrectRootError);
				}
		...
		try {
			...
			PLMDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		...
		protected boolean validatePage() {
			...
			boolean result = ViewService.getInstance()
					.provides(new CreateDiagramViewOperation(new EObjectAdapter(getModelElement()),
							DomainEditPart.MODEL_ID, PLMDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT));
			setErrorMessage(result ? null : Messages.PLMNewDiagramFileWizard_RootSelectionPageInvalidSelectionMessage);
			return result;
		}
	}
}
