...
public class EntityCreateCommand extends EditElementCommand {
  ...
  public boolean canExecute() {
    ...
    if (type.eContainer() instanceof Level || type.eContainer() instanceof Package
        && type.getComposite() == null && getElementToEdit() instanceof Level)
      return true;
    ...
    if ((type.getComposite() != null || (type.getAggregates().size() > 0))
      ...
      if (type.getComposite() != null && ((Clabject) getElementToEdit())
          .getClassificationTreeAsInstance().contains((Clabject) type.getComposite()))
        return true;
  ...
  protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
    ...
    if (getRequest()
        .getParameter(ModelEditorConstants.CREATE_COMMAND_PRECONFIGURED_ELEMENT) != null)
      newElement = (Entity) getRequest()
    ...
    ElementInitializers.getInstance().init_Entity_3105(newElement);

    doConfigure(newElement, monitor, info);

    // Set the location
    if (getRequest().getParameter(ModelEditorConstants.CREATE_COMMAND_LOCATION) != null) {
      Point location = (Point) getRequest()
          .getParameter(ModelEditorConstants.CREATE_COMMAND_LOCATION);
