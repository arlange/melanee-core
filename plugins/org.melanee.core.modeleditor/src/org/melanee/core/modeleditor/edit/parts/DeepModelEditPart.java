...
public class DeepModelEditPart extends ShapeNodeEditPart {
  ...
  protected void createDefaultEditPolicies() {
    ...
    installEditPolicy(EditPolicy.PRIMARY_DRAG_ROLE,
        new OneDirectionResizableEditPolicy(PositionConstants.EAST));
  ...
  protected boolean addFixedChild(EditPart childEditPart) {
    if (childEditPart instanceof DeepModelNameEditPart) {
      ((DeepModelNameEditPart) childEditPart)
          .setLabel(getPrimaryShape().getFigureDeepModelNameFigure());
      return true;
    }
    if (childEditPart instanceof WrappingLabelEditPart) {
      ((WrappingLabelEditPart) childEditPart)
          .setLabel(getPrimaryShape().getFigureDeepModelComplianceWarning());
      return true;
    }
    if (childEditPart instanceof WrappingLabel2EditPart) {
      ((WrappingLabel2EditPart) childEditPart)
          .setLabel(getPrimaryShape().getFigureDeepModelAVSFigure());
      return true;
    }
    ...
    if (request instanceof CreateViewAndElementRequest) {
      ...
      if (type == PLMElementTypes.Enumeration_3123) {
        return getChildBySemanticHint(PLMVisualIDRegistry
            .getType(DeepModelDeepModelEnumerationCompartmentEditPart.VISUAL_ID));
      }
    }
  ...
  public class DeepModelFigure extends RoundedRectangle {
    ...
    private void createContents() {
      ...
      fFigureDeepModelEnumerations.setBorder(new MarginBorder(getMapMode().DPtoLP(0),
  ...
  public Command getCommand(Request _request) {
    ...
    if (cmd == null && _request instanceof CreateUnspecifiedTypeRequest) {
      // Hand over requests which cannot be handled to the LevelComparment. Allows
      // adding
