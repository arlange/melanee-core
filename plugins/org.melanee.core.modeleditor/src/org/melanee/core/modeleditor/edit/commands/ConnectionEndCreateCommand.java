...
public class ConnectionEndCreateCommand extends AbstractImplicitMiddleCreateCommand {
	 ...
	 * @generated
	 */
	public ConnectionEndCreateCommand(CreateRelationshipRequest request, EObject source, EObject target) {
	...
	protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
		...
		if (getRequest().getParameter(ImplicitConnectionGraphicalNodeEditPolicy.DSL_TYPE) instanceof ConnectionEnd
	 ...
	 * @generated
	 */
	protected CommandResult doExecuteWithResultImplicitMiddle(IProgressMonitor monitor, IAdaptable info)
		...
		if (getRequest().getParameter(ImplicitConnectionGraphicalNodeEditPolicy.DSL_TYPE) != null)
			instantiateDSLValues(
					(Connection) getRequest().getParameter(ImplicitConnectionGraphicalNodeEditPolicy.DSL_TYPE),
					(Connection) middle);
	...
	private boolean isDirected(BasicEList<EObject> connectionEnds) {
		...
		return ((ConnectionEnd) connectionEnds.get(0)).isNavigable() != ((ConnectionEnd) connectionEnds.get(1))
				.isNavigable();
	}
	 ...
	 * @generated
	 */
	private ConnectionEnd getConnectionEndBasedOnType(BasicEList<EObject> connectionEnds, Clabject instance) {
	 ...
	 * @generated
	 */
	private ConnectionEnd getConnectionEndBasedOnDirection(BasicEList<EObject> connectionEnds, boolean navigable) {
		Assert.isTrue(connectionEnds.size() == 2);
	...
	private void instantiateDSLValues(Connection typeConnection, Connection instanceConnection) {
		...
		BasicEList<EObject> typeConnectionEnds = new BasicEList<EObject>(typeConnection.getAllConnectionEnd());
		ConnectionEnd[] typeConnectionEnd = new ConnectionEnd[2]; // right now the graphical editor only allows to create a new connection between exactly two entities in a single user action
		...
		for (int i = 0; i < typeConnectionEnds.size(); ++i) {
			...
			ConnectionEnd instanceConnectionEnd = getConnectionEndToDestination(instanceConnection, destination);
	...
	protected IElementTypeAwareAdapter createSemanticMiddle(IProgressMonitor monitor, IAdaptable info)
		...
		//The container is a model
			...
			Connection c = PLMFactory.eINSTANCE.createConnection();
			result = new EObjectAndElementTypeAdapter(c, PLMElementTypes.Connection_3099, ConnectionEditPart.VISUAL_ID);
			invokeDefaultElementTypeConfiguration(result, monitor, info);
	 ...
	 */
	@Override
	protected IElementTypeAwareAdapter createImplicitTargetLink(IProgressMonitor monitor, IAdaptable info)
			throws ExecutionException {

		implicitConnectionEnd = PLMFactory.eINSTANCE.createConnectionEnd();
		return new EObjectAndElementTypeAdapter(implicitConnectionEnd, PLMElementTypes.ConnectionEnd_4036,
