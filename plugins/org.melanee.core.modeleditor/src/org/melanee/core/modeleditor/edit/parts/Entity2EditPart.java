...
public class Entity2EditPart extends AbstractBorderedShapeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof WrappingLabel35EditPart) {
			((WrappingLabel35EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityNameFigure());
			return true;
		}
		if (childEditPart instanceof WrappingLabel36EditPart) {
			((WrappingLabel36EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityPotencyLabel());
			return true;
		}
		if (childEditPart instanceof WrappingLabel37EditPart) {
			((WrappingLabel37EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityLevelFigure());
			return true;
		}
		if (childEditPart instanceof WrappingLabel38EditPart) {
			((WrappingLabel38EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityAVSFigure());
			return true;
		}
	...
	public void makeCompartmentsVisible(int compartments) {
		...
		try {
			...
			else if (self != null && self.getDefinedAttributes().size() == 0 && getAttributesCompartment() != null)
	...
	public void updateDSL(String format, String notation) {
		...
		if (format.equals(IVisualizationServiceBase.FORMAT_GRAPH)) {
			...
			AbstractDSLVisualizer dslVisualizer = null;
			try {
				transformator = ExtensionPointService.Instance().getActiveGraphicalVisualizationService();
				dslVisualizer = transformator.findDSLVisualizerForElement(self, notation, true);
	 ...
	 * 
	 * @param notationModel True -> write from notation model to visualizer; false
	 *                      -> write from visualizer to notation model
	 */
