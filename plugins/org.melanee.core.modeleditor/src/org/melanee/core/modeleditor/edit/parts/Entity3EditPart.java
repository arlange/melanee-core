...
import org.melanee.core.modeleditor.edit.policies.Entity3ItemSemanticEditPolicy;
...
public class Entity3EditPart extends AbstractBorderedShapeEditPart {
	...
	*/
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(EditPolicyRoles.SEMANTIC_ROLE, new Entity3ItemSemanticEditPolicy());
	...
	*/
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof WrappingLabel40EditPart) {
			((WrappingLabel40EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityNameFigure());
			return true;
		}
		if (childEditPart instanceof WrappingLabel41EditPart) {
			((WrappingLabel41EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityPotencyLabel());
			return true;
		}
		if (childEditPart instanceof WrappingLabel42EditPart) {
			((WrappingLabel42EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityLevelFigure());
			return true;
		}
		if (childEditPart instanceof WrappingLabel43EditPart) {
			((WrappingLabel43EditPart) childEditPart).setLabel(getPrimaryShape().getFigureEntityAVSFigure());
			return true;
		}
		...
		if (childEditPart instanceof EntityEntityAttributesCompartment3EditPart) {
			IFigure pane = getPrimaryShape().getFigureAttributesCompartmentRectangle();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((EntityEntityAttributesCompartment3EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof EntityEntityMethodsCompartment3EditPart) {
			IFigure pane = getPrimaryShape().getFigureMethodsCompartmentRectangle();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((EntityEntityMethodsCompartment3EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof EntityEntityContentsCompartment3EditPart) {
			IFigure pane = getPrimaryShape().getFigureContentCompartmentRectangle();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((EntityEntityContentsCompartment3EditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	*/
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof EntityEntityAttributesCompartment3EditPart) {
			IFigure pane = getPrimaryShape().getFigureAttributesCompartmentRectangle();
			pane.remove(((EntityEntityAttributesCompartment3EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof EntityEntityMethodsCompartment3EditPart) {
			IFigure pane = getPrimaryShape().getFigureMethodsCompartmentRectangle();
			pane.remove(((EntityEntityMethodsCompartment3EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof EntityEntityContentsCompartment3EditPart) {
			IFigure pane = getPrimaryShape().getFigureContentCompartmentRectangle();
			pane.remove(((EntityEntityContentsCompartment3EditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	*/
	protected IFigure getContentPaneFor(IGraphicalEditPart editPart) {
		if (editPart instanceof EntityEntityAttributesCompartment3EditPart) {
			return getPrimaryShape().getFigureAttributesCompartmentRectangle();
		}
		if (editPart instanceof EntityEntityMethodsCompartment3EditPart) {
			return getPrimaryShape().getFigureMethodsCompartmentRectangle();
		}
		if (editPart instanceof EntityEntityContentsCompartment3EditPart) {
	...
	*/
	protected void addBorderItem(IFigure borderItemContainer, IBorderItemEditPart borderItemEditPart) {
		if (borderItemEditPart instanceof ConnectionNameExternalLabel3EditPart) {
	...
	*/
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(PLMVisualIDRegistry.getType(WrappingLabel40EditPart.VISUAL_ID));
	}
	...
	*/
		...
		if (notification.getNotifier() instanceof Element) {
				...
				for (Clabject instance : changedEntity.getInstances()) {
					...
					if (featureEditParts.size() > 0) {
						...
						if (featureEditPart instanceof EntityEditPart)
							...
							((org.melanee.core.modeleditor.edit.parts.Entity3EditPart) featureEditPart)
									.updateView(IVisualizationServiceBase.FORMAT_GRAPH);
					}
				}
			}
			...
			if (changedFeature.getName().equals("name")) {
				...
				try {
					...
					feature = ((HashSet<Feature>) ocl.evaluate(EcoreUtil.getRootContainer(resolveSemanticElement()), q))
							.toArray(new Feature[] {});
				} catch (ParserException e) {
					e.printStackTrace();
				}
	...
	private void updateNameCompartment() {
		try {
			...
			WrappingLabel classificationDesignationWrappingLabel =
					...
					((org.melanee.core.modeleditor.edit.parts.Entity3EditPart.EntityFigure) getPrimaryShape())
	...
	public void makeCompartmentsVisible(int compartments) {
		...
		Map<java.lang.String, Boolean> commandOptions = Collections.singletonMap(Transaction.OPTION_NO_UNDO,
				Boolean.TRUE);
	...
	public void updateDSL(String format, String notation) {
		...
		if (format.equals(IVisualizationServiceBase.FORMAT_GRAPH)) {
			...
			AbstractDSLVisualizer dslVisualizer = transformator.findDSLVisualizerForElement(self, notation, true);
			...
			BorderItemContainerFigure sourceBICF = (BorderItemContainerFigure) figure.getChildren().get(1);
			...
			if (customFigure instanceof DefaultSizeNodeFigure) {
				...
				BorderItemContainerFigure targetBICF = (BorderItemContainerFigure) customFigure.getChildren().get(1);
				...
				for (IFigure f : (List<IFigure>) targetBICF.getChildren()) {
					...
					CenteredBorderItemLocator oldLocator = (CenteredBorderItemLocator) (f.getParent().getLayoutManager()
							.getConstraint(f) instanceof CenteredBorderItemLocator
									? f.getParent().getLayoutManager().getConstraint(f) : null);
		...
		public void notifyChanged(Notification notification) {
			...
			for (Clabject i : instances) {
				...
				if (ep instanceof EntityEditPart)
					...
					((org.melanee.core.modeleditor.edit.parts.Entity3EditPart) ep)
							.updateView(IVisualizationServiceBase.FORMAT_GRAPH);
