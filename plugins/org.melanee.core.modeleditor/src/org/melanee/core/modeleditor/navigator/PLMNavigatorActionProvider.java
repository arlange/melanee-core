 ...
 * @generated
 */
public class PLMNavigatorActionProvider extends CommonActionProvider {
	...
	private static class OpenDiagramAction extends Action {
		...
		*/
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof PLMNavigatorItem) {
					selectedElement = ((PLMNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (DomainEditPart.MODEL_ID.equals(PLMVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		*/
		public void run() {
			...
			try {
				page.openEditor(editorInput, PLMDiagramEditor.ID);
			} catch (PartInitException e) {
				PLMDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
