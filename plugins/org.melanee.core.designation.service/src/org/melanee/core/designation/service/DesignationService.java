...
public class DesignationService implements IDesignationService {
  ...
  public String evaluate(String query, Clabject context, boolean heritage, boolean location,
      boolean identifier, boolean classification) {

    String result = "";

    StringBuilder builder = new StringBuilder(query);
    Map<Integer, String> subDesignators = cleanControlLanguageFromSubDesignators(builder, context,
  ...
  private Map<Integer, String> cleanControlLanguageFromSubDesignators(StringBuilder builder,
      Clabject context, Map<Integer, String> subDesignators, int subDesignatorID) {
    int startIndex = -1;
    int endIndex = -1;
    int level = 0;
  ...
  private String createHeritage(String query, Clabject context,
      Map<Integer, String> subDesignators) {
    String result = "";

    // Keep right group in sync with variable
    String regex = "(?<left>\\d+)(?<subdesignator>\\$.*?\\$)?(?<operator>\\" + HERITAGE_OPERATOR
        + "(\\" + HERITAGE_OPERATOR + ")?)(?<right>[\\d+([a-zA-Z_]+)])";
    ...
    while (m.find(start)) {
      ...
      ParseResult parseResult = parseExpression(m, HERITAGE_OPERATOR,
          HERITAGE_OPERATOR + HERITAGE_OPERATOR, 3, true, subDesignators);

      // the next search shall start after the operator
      start = parseResult.getOperatorPosition();

      // Right is location
      Pattern rightIsLocation = Pattern.compile(
          "(?<location>\\d+[\\" + LOCATION_OPERATOR + "(\\" + LOCATION_OPERATOR + ")\\d+])");
      Matcher rightIsLocationMatcher = rightIsLocation
          .matcher(query.substring(m.start(rightGroup)));

      int right = parseResult.getRightAsInteger();
      if (rightIsLocationMatcher.find() && rightIsLocationMatcher.start() == 0)
        right = 0;
      ...
      Map<Integer, Set<Element>> distanceGroups = getIdentifiersInRange(right,
          parseResult.getLeftAsInteger(), 0, parseResult.isEllision, context,
          new HashMap<Integer, Set<Element>>(), DesignationService.HERITAGE);
  ...
  private String createLocation(String query, Clabject context,
      Map<Integer, String> subDesignators) {
    String result = "";

    String regex = "(?<left>\\d+)(?<subdesignator>\\$.*?\\$)?(?<operator>\\" + LOCATION_OPERATOR
        + "(\\" + LOCATION_OPERATOR + ")?)(?<right>[\\d+([a-zA-Z_]+)])";
    Pattern p = Pattern.compile(regex);
    Matcher m = p.matcher(query);
    ...
    while (m.find(start)) {
      ...
      ParseResult parseResult = parseExpression(m, LOCATION_OPERATOR,
          LOCATION_OPERATOR + LOCATION_OPERATOR, 3, true, subDesignators);
      ...
      Map<Integer, Set<Element>> distanceGroups = getIdentifiersInRange(
          parseResult.getRightAsInteger(), parseResult.getLeftAsInteger(), 0,
          parseResult.isEllision, context, new HashMap<Integer, Set<Element>>(),
          DesignationService.LOCATION);
      result = getDesignatorSubString(distanceGroups, parseResult, result, true);
    }
  ...
  private String createClassification(String query, Clabject context,
      Map<Integer, String> subDesignators) {
    String result = "";

    // Keep right group in sync with variable
    String regex = "(?<left>[\\d+([a-zA-Z_]+)])(?<operator>" + CLASSIFICATION_OPERATOR + "("
        + CLASSIFICATION_OPERATOR + ")?)(?<right>\\d+)(?<subdesignator>\\$.*?\\$)?";
    ...
    while (m.find(start)) {
      ...
      ParseResult parseResult = parseExpression(m, CLASSIFICATION_OPERATOR,
          CLASSIFICATION_OPERATOR + CLASSIFICATION_OPERATOR, 2, false, subDesignators);
      ...
      Map<Integer, Set<Element>> distanceGroups = getIdentifiersInRange(
          parseResult.getLeftAsInteger(), parseResult.getRightAsInteger(), 0,
          parseResult.isEllision, context, new HashMap<Integer, Set<Element>>(),
          DesignationService.CLASSIFICATION);
  ...
  private String getDesignatorSubString(Map<Integer, Set<Element>> distanceGroup,
      ParseResult parseResult, String result, boolean leftOfDesignator) {
    ...
    for (int i = start;;) {
      ...
      if (!parseResult.getOperator().contains(CLASSIFICATION_OPERATOR) && (i < 0))
        break;
      // Abort conditon classification
      else if (parseResult.getOperator().contains(CLASSIFICATION_OPERATOR) && i >= distances.length)
        break;
      ...
      for (Element e : list) {
        if (!parseResult.hasSubDesignator())
          ...
          tmp += "(" + DesignationService.getInstance().evaluate((Clabject) e,
              parseResult.getSubDesignator()) + "),";
      ...
      String[] results = result
          .split(parseResult.getOperatorEscaped() + "(" + parseResult.getOperatorEscaped() + ")?");
      ...
      if (!added) {
        if (!leftOfDesignator)
          result += parseResult.isEllision ? parseResult.getEllisionOperator()
              : parseResult.getOperator();
        result += tmp;
        if (leftOfDesignator)
          result += parseResult.isEllision ? parseResult.getEllisionOperator()
              : parseResult.getOperator();
      ...
      // last sign correct.
      else if (leftOfDesignator && result.endsWith(parseResult.getOperator())) {
  ...
  private String getIdentifier(String query, Clabject context,
      Map<Integer, String> subDesignators) {
    ...
    while (m.find()) {
      ...
      if (a == null)
        return context.getName() == null || context.getName().equals("") ? ANONYMOUS_IDENTIFIER
            : context.getName();
  ...
  private Map<Integer, Set<Element>> getIdentifiersInRange(int startDistance, int endDistance,
      int currentDistance, boolean elide, Element context,
      Map<Integer, Set<Element>> distanceGroups, int searchFor) {
    currentDistance++;

    // Everything between start and end needs to be shown
    if (currentDistance >= startDistance && currentDistance <= endDistance && !elide) {
      ...
      for (Element e : queryResult)
        getIdentifiersInRange(startDistance, endDistance, currentDistance, elide, e, distanceGroups,
            searchFor);
    }
    // only start and end need to be shown
    else if ((currentDistance == startDistance || currentDistance == endDistance) && elide) {
      Set<Element> queryResult = new HashSet<Element>();
      if (searchFor == DesignationService.HERITAGE)
        queryResult.addAll(((Clabject) context).getDirectSupertypes());
      else if (searchFor == DesignationService.LOCATION)
  ...
  /**
   * This contains the common code for parsing query in the form
   * <left><operator><right>
   * 
   * @param m
   *          the matcher to use
   ...
   * @param ellisionOperator
   *          the ellisionOperator that is used
   * @param operatorGroupNumber
   *          the group number of the operator in the matcher
   * @param identifierRight
   *          is the query left of the operator?
   * 
   * @return returns a parse result containing the obtained information
   */
  private ParseResult parseExpression(Matcher m, String operator, String ellisionOperator,
      int operatorGroupNumber, boolean identifierRight, Map<Integer, String> subDesignators) {
    ...
    ParseResult p = new ParseResult(left, right, operator, isEllision, isIdentifier,
        operatorPosition, identifierRight, ellisionOperator, hasSubDesignator, subDesignator);
  ...
  protected class ParseResult {
    ...
    /**
     * 
     * @return the string as integer; if the identifier is on this side 0 is
     *         returned
     */
    public int getLeftAsInteger() {
      if (!isIdentifierRight && isIdentifier)
        return 0;
    ...
    public ParseResult(String left, String right, String operator, boolean isEllision,
        boolean isIdentifier, int operatorPosition, boolean isIdentifierRight,
        String ellisionOperator, boolean hasSubDesignator, String subDesignator) {
      this.left = left;
      this.right = right;
