...
public class NnaryLayouter {
  ...
  public boolean matchesPattern(List<Rectangle> rectangles) {
     ...
     * |__| | | |__| | |_________| |_________| | | _________ _________ | |__| | |
     * |__| | |_________| |_________| | | _________ _________ | |__| | | |__|
     * |_________| |_________|
     * 
     * _________ _________ | |__ __| | |_________| | | |_________| _________ | |
     * _________ | |__| |__| | |_________| | | |_________| _________ | | _________ |
     * |__| |__| | |_________| | | |_________| ____|____ ____|____ | | | |
     * |_________| |_________|
     */
     ...
     * |_________| |_________| |_________| ____________|____________
     * |____________|____________| ____|____ ____|____ ____|____ ____|____ | | | | |
  ...
  public Point calculateNewMiddle() {
    ...
    /*
     * _________ | | |_________| ____________|____________ ____|____ ____|____
    ...
    if (pattern == PATTERN_HORIZONTAL_NORTH_TREE) {
      ...
      for (Rectangle rect : rectangles)
        if (rect == north)
          continue;
        else if (currentClosest == null)
          currentClosest = rect;
        else if (Math.abs(rect.getCenter().x - north.getCenter().x) < Math
            .abs(currentClosest.getCenter().x - north.getCenter().x))
          currentClosest = rect;

      return new Point((currentClosest.getCenter().x + north.getCenter().x) / 2,
          (currentClosest.getCenter().y + north.getCenter().y) / 2);

    }

    /*
     * _________ _________ _________ | | | | | | |_________| |_________| |_________|
     * |____________|____________| ____|____ | | |_________|
     * 
     */
    if (pattern == PATTERN_HORIZONTAL_SOUTH_TREE) {
      ...
      for (Rectangle rect : rectangles)
        if (rect == south)
          ...
          currentClosest = rect;
        else if (Math.abs(rect.getCenter().x - south.getCenter().x) < Math
            .abs(currentClosest.getCenter().x - south.getCenter().x))
          currentClosest = rect;

      return new Point((currentClosest.getCenter().x + south.getCenter().x) / 2,
          (currentClosest.getCenter().y + south.getCenter().y) / 2);
    }
