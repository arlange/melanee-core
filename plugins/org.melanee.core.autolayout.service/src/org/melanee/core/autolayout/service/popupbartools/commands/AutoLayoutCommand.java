...
public class AutoLayoutCommand extends Command {
  ...
  public void execute() {
    ...
    if (host.resolveSemanticElement() instanceof Connection
      ...
      for (Object connectionEndEditPart : host.getSourceConnections()) {
        connectedClientAreas
            .add(((IGraphicalEditPart) ((ConnectionNodeEditPart) connectionEndEditPart).getTarget())
                .getFigure().getClientArea());
      }
    ...
    SetBoundsCommand sbc = new SetBoundsCommand(host.getEditingDomain(), "AutoLayout",
        new EObjectAdapter(host.getNotationView()), newPosition);
   ...
   */
  private static void cleanAnchorLocations(IGraphicalEditPart connection,
      Point newConnectionCenter) {

    for (ConnectionNodeEditPart connectionEnd : (List<ConnectionNodeEditPart>) connection
      ...
      if (clabjectRectangle.getCenter().y < newConnectionCenter.y) {
        ...
        if (clabjectRectangle.getCenter().x > leftXBorder
            && clabjectRectangle.getCenter().x < rightXBorder) {
          SetConnectionAnchorsCommand sca = new SetConnectionAnchorsCommand(
              connection.getEditingDomain(), "Rearrange Anchor");
          ...
          connectionEnd.getViewer().getEditDomain().getCommandStack()
              .execute(new ICommandProxy(sca));
