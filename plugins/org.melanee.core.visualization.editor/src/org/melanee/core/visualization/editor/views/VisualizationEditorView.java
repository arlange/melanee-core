		...
		if (propertySheetPage == null) {
			...
			if (getSite().getWorkbenchWindow().getActivePage().getActiveEditor() == null ||
				...
				if(! getSelection().isEmpty())
					domain =  TransactionUtil.getEditingDomain((EObject)((IStructuredSelection)getSelection()).getFirstElement());
				else if (viewer.getTree().getItemCount() > 0)
					domain = TransactionUtil.getEditingDomain((EObject)viewer.getTree().getItem(0).getData());
				else
					return null;
...
//			viewer.getSelection();
			...
			propertySheetPage =
				new MyExtendedPropertySheetPage((AdapterFactoryEditingDomain)domain);
	...
	/**
	 * This class is needed to prevent sorting of RGB color Attributes.
	 * Everything else is sorted using the default behaviour.
	 */
	private class MyExtendedPropertySheetPage extends
		...
		/**
		 * This is used to not sort RGB color properties
		 * alphabetically
		 */
		@Override
			...
			sorter = new PropertySheetSorter(){
				...
				public void sort(IPropertySheetEntry[] entries) {
					...
					for (IPropertySheetEntry entry :entries){
						isRGB = entry.getDisplayName().equals("R")
								|| entry.getDisplayName().equals("G")
	...
	public VisualizationEditorView() throws CoreException {
		...
		if (ExtensionPointService.Instance().getActiveGraphicalVisualizationService() != null)
			activeVisualizationServices.add(ExtensionPointService.Instance().getActiveGraphicalVisualizationService());
		if (ExtensionPointService.Instance().getActiveTextualVisualizationService() != null)
			activeVisualizationServices.add(ExtensionPointService.Instance().getActiveTextualVisualizationService());
		if (ExtensionPointService.Instance().getActiveFormVisualizationService() != null)
			activeVisualizationServices.add(ExtensionPointService.Instance().getActiveFormVisualizationService());
		if (ExtensionPointService.Instance().getActiveValidationService() != null)
			activeVisualizationServices.add(ExtensionPointService.Instance().getActiveValidationService());
		if (ExtensionPointService.Instance().getActiveApplicationVisualizationService() != null)
			activeVisualizationServices.add(ExtensionPointService.Instance().getActiveApplicationVisualizationService());
		if (ExtensionPointService.Instance().getActiveTableVisualizationService() != null)
			activeVisualizationServices.add(ExtensionPointService.Instance().getActiveTableVisualizationService());
	}
...
//				return 
					//Graphical Level
//						(element instanceof GraphicalDSLVisualizer) || (element instanceof LMLVisualizer)  
...
//					|| (element instanceof LinkDecoration) || (element instanceof SpacingDescriptor)
					// Textual Level
//					|| (element instanceof TextualDSLVisualizer) || (element instanceof TextualVisualizationDescriptor)
...
//		getSite().getPage().addSelectionListener(PLMDiagramEditor.ID ,this);
	...
	ISelectionChangedListener menuBuilder = new ISelectionChangedListener() {
		 ...
		 * visualization meta-models which reference the PLM. Thus these types
		 * are added manually to the childe menu with this method. This is done
		 * by searching for meta-models provided through IVisualizationServiceBase
		 * in future releases.
		 * 
		 * @param object The owner of the newly created model element
		 * @return an array with command parameters for all DSL extensions
		 */
		private Collection<?> getDomainSpecificVisualizerChildMenuEntries(
				Object object) {
			...
			for(IVisualizationServiceBase visualizationService : activeVisualizationServices){
				CommandParameter parameter = new CommandParameter(object, PLMPackage.eINSTANCE.getLMLVisualizer_DslVisualizer(), visualizationService.createNewSpecificDSLVisualizer());
