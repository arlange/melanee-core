...
public class EmendChangeFeatureTraitCommandFactory{
	...
	public EmendChangeFeatureTraitCommandFactory(IImpactAnalyzer impactAnalyzer, Feature changeOrigin, EStructuralFeature traitToChange, String oldValue, String newValue){
	...
	protected EmendChangeTraitDialog showChangeValueDialog(String valueToDisplayValue, String oldValue, Feature changeOrigin) throws ExecutionException{
		EmendChangeTraitDialog dialog = new EmendChangeTraitDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), valueToDisplayValue, EmendChangeTraitDialog.FEATURE);
	...
	protected CompoundCommand runEmendation(){
		emendedElements = impactAnalyzer.getAffectedModelElementsForChangeTraitFeatureOperation(changeOrigin, oldValue, traitToChange);
		...
		for (Feature element : emendedElements)
			if (traitToChange.getEType().getName().equals("EInt")){
				int computedValue = (Integer) computeNewValue(changeOrigin, element, newValue, traitToChange);
...
//		domain.getCommandStack().execute(emendationCommand);
		...
		//	emendationCommand = new CompoundCommand("Emendation (Value) - " + traitToChange.getName() + " - " + changeOrigin.getName());
//			
//			for (Feature element : emendedElements){
//				//For mutability 0 we need to change the value
...
//							"		allDirectSupertypes.getAllFeatures()->select(f | f.name = self.name and f.oclIsTypeOf(Attribute))->first().oclAsType(Attribute) " +
//							"	in " +
...
//								|| ((((Attribute)element).getValue() != null && !((Attribute)element).getValue().equals(validValue)))
//							){
//							emendationCommand.append(SetCommand.create(domain, element, PLMPackage.eINSTANCE.getAttribute_Value(), validValue));
//							domain.getCommandStack().execute(emendationCommand);
//							emendationCommand = new CompoundCommand("Emendation (Value) - " + traitToChange.getName() + " - " + changeOrigin.getName());
//						}
//			
...
//		}
	...
	protected Object computeNewValue(Feature changeOrigin, Feature machtingElement, String newValue, EStructuralFeature traitToChange){
	...
	protected boolean isPotencyValue(EStructuralFeature traitToChange){
		return traitToChange == PLMPackage.eINSTANCE.getFeature_Durability() || traitToChange == PLMPackage.eINSTANCE.getAttribute_Mutability();
	}
}