...
public class EmendMoveAttributeCommandFactory{
	...
	public EmendMoveAttributeCommandFactory(IImpactAnalyzer impactAnalyzer, Clabject source, Clabject target, Feature movedFeature){
	...
	protected EmendMoveAttributeDialog showMoveModelElementDialog() throws ExecutionException{
		EmendMoveAttributeDialog dialog = new EmendMoveAttributeDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell());
...
//		Set<Clabject> emendedElements = new ImpactAnalyzer().getAffectedModelElementsForAddFeatureOperation(changeOrigin); //(Set<Clabject>)new ImpactAnalyzer().calculateImpactOfChange((Clabject)changeOrigin, traitToChange, changeOntologicalTypes, changeSubtypes, changeSuperTypes);

		CompoundCommand emendationCommand = new CompoundCommand("Emendation move feature " + movedFeature.getName() + " from " + source.getName() + " to " + target.getName());
		
		EmendRemoveAttributeCommandFactory removeAttributeCommand = new EmendRemoveAttributeCommandFactory(impactAnalyzer, source, movedFeature); 
		Command cmd = removeAttributeCommand.run(impactAnalyzer.getAffectedModelElementsForAddFeatureOperation(target, movedFeature, true));
		if (cmd != null)
			emendationCommand.append(cmd);
		EmendAddAttributeCommandFactory addAttributeCommand = new EmendAddAttributeCommandFactory(impactAnalyzer, target, movedFeature);
