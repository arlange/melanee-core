...
public class EmendAddAttributeCommandFactory {
  ...
  public EmendAddAttributeCommandFactory(IImpactAnalyzer impactAnalyzer, Clabject changeOrigin,
  ...
  public CompoundCommand run() {
    try {
      ...
      return runEmendation(dialog.getNewName(), dialog.getNewDataType(), dialog.getNewDurability(),
          dialog.getNewMutability(), dialog.getPreventDuplicates(), dialog.getAddToSupertypesOnly(),
          dialog.getAddToOntologicalTypes(), dialog.getAddToOntologicalInstances());
  ...
  protected EmendAddAttributeDialog showAddModelElementDialog(Feature addedFeature)
      throws ExecutionException {
    EmendAddAttributeDialog dialog = new EmendAddAttributeDialog(
   ...
   * @return
   */
  protected CompoundCommand runEmendation(String name, String dataType, int durability,
      int mutability, boolean preventDuplicates, boolean addToSuperTypesOnly,
      boolean includeOntologicalTypes, boolean includeOntologicalInstances) {
    emendedElements = impactAnalyzer.getAffectedModelElementsForAddFeatureOperation(changeOrigin,
        addedFeature, name, !preventDuplicates, addToSuperTypesOnly, includeOntologicalTypes,
        includeOntologicalInstances);

    CompoundCommand emendationCommand = new CompoundCommand(
        "Emendation - Add Feature" + addedFeature.getName());
    TransactionalEditingDomain domain = TransactionUtil.getEditingDomain(changeOrigin);

    // ***********************************
    // Setup the via UI added element
    // ***********************************
    Command setOriginName = SetCommand.create(domain, addedFeature,
        PLMPackage.eINSTANCE.getElement_Name(), name);
    emendationCommand.append(setOriginName);

    // If no datatype was selected no datatype is set
    if (!"".equals(dataType)) {
      Command setOriginDataType = SetCommand.create(domain, addedFeature,
          PLMPackage.eINSTANCE.getAttribute_Datatype(), dataType);
      emendationCommand.append(setOriginDataType);
    }

    Command setOriginDurability = SetCommand.create(domain, addedFeature,
        PLMPackage.eINSTANCE.getFeature_Durability(), durability);
    emendationCommand.append(setOriginDurability);
    Command setOriginMutabilityCommand = SetCommand.create(domain, addedFeature,
        PLMPackage.eINSTANCE.getAttribute_Mutability(), mutability);
    emendationCommand.append(setOriginMutabilityCommand);

    // This is only used as template for the configure method of PLMFactory
    Feature template = addedFeature instanceof Attribute ? PLMFactory.eINSTANCE.createAttribute()
    ...
    for (Clabject element : emendedElements)
      if (element != changeOrigin) {
        Feature newFeature = PLMFactory.eINSTANCE.createFeature(template,
            addedFeature.getClabject(), element, false);

        emendationCommand.append(AddCommand.create(domain, element,
            PLMPackage.eINSTANCE.getClabject_Feature(), newFeature));
      }
