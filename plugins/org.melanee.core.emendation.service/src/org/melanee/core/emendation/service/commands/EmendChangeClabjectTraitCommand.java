...
public class EmendChangeClabjectTraitCommand {
  ...
  public EmendChangeClabjectTraitCommand(IImpactAnalyzer impactAnalyzer, Clabject changeOrigin,
  ...
  protected EmendChangeTraitDialog showChangeValueDialog(String valueToDisplayValue,
      String oldValue, Clabject changeOrigin) throws ExecutionException {
    EmendChangeTraitDialog dialog = new EmendChangeTraitDialog(
        ...
        EmendChangeTraitDialog.CLABJECT);
  ...
  protected CompoundCommand runEmendation(boolean includeNonShallowInheritance) {
    emendedElements = impactAnalyzer.getAffectedModelElementsForChangeTraitClabjectOperation(
        changeOrigin, oldValue, traitToChange, includeNonShallowInheritance); // (Set<Clabject>)new
                                                                              // ImpactAnalyzer().calculateImpactOfChange((DomainElement)changeOrigin,
                                                                              // oldValue,
                                                                              // traitToChange,
    ...
    for (Clabject element : emendedElements)
      if (traitToChange.getEType().getName().equals("EInt"))
        emendationCommand.append(SetCommand.create(domain, element, traitToChange,
            (Integer) computeNewValue(changeOrigin, element, newValue, traitToChange)));
      else
        emendationCommand.append(SetCommand.create(domain, element, traitToChange,
  ...
  protected Object computeNewValue(Clabject changeOrigin, Clabject machtingElement, String newValue,
