...
public class EmendRemoveAttributeCommandFactory {
  ...
  public EmendRemoveAttributeCommandFactory(IImpactAnalyzer impactAnalyzer, Clabject changeOrigin,
  ...
  protected EmendRemoveAttributeDialog showRemoveModelElementDialog() throws ExecutionException {
    EmendRemoveAttributeDialog dialog = new EmendRemoveAttributeDialog(
   ...
   * @return
   */
  protected CompoundCommand runEmendation(Clabject changeOrigin, Feature featureToDelete,
      Set<Clabject> exclude) {
    emendedElements = impactAnalyzer
        .getAffectedModelElementsForDeleteFeatureOperation(featureToDelete, changeOrigin);// (Set<Feature>)new
                                                                                          // ImpactAnalyzer<Feature>().calculateImpactOfChange((Feature)featureToDelete,
                                                                                          // String.valueOf(featureToDelete.getDurability()),
                                                                                          // PLMPackage.eINSTANCE.getFeature_Durability(),
                                                                                          // changeOntologicalTypes,
                                                                                          // changeSubtypes,
                                                                                          // changeSuperTypes);
    ...
    // ***************************************************************
    final CompoundCommand emendationCommand = new CompoundCommand(
        "Emendation - Delete " + changeOrigin.getName());
    ...
    // This is needed because if we place an empty compound command in a compound
    // command
    // this makes problems during execution of following commands. e.g.
    // EmmendMoveCommand
