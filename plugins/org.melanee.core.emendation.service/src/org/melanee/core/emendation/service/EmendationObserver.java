...
/**
 * The emendation obeserver is responsible for detecting the different
 * classes of changes made to a model (e.g. change trait, delete clabject).
 * After detecting these changes, a call to the active emendation service
 * is made which supports the user in doing changes to the model. The 
 * emendation observer comes with a default emendation service in case
 * that no emendation service is provided.
 *
 */
public class EmendationObserver implements ResourceSetListener {
	...
	/**
	 * This is stores the emended model elements. After emendation the service
	 * is called a second time as all emended model elements are changes :).
	 * The service then checks if called if the model element was changed by the
	 * service (in the set) or user (not in the set). If the model element is 
	 * changed by the service no emendation is executed.
	 */
	Set<EObject> emendedObjects = new HashSet<EObject>();
	
	/**
	 * Starts listening for changes to the P_EMENDATION_ENGINE_ACTIVATED
	 * setting.
	 */
	private void listenForActivateEmendationServicePropertyChange(){
		
		runService = org.melanee.core.workbench.Activator.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.P_EMENDATION_ENGINE_ACTIVATED);
		
		org.melanee.core.workbench.Activator.getDefault().getPreferenceStore().addPropertyChangeListener(
	 ...
	 * are still listened aftre listening to a new model. Use stopListening(modelRoot)
	...
	public Command transactionAboutToCommit(ResourceSetChangeEvent event)
		...
		if (removeNotification != null
			...
			//Collection<EObject> affectedModelElements = analyzer.getAffectedModelElementsForMoveFeatureOperation((Clabject)removeNotification.getNotifier(), (Clabject)addNotification.getNotifier(), (Feature)addNotification.getNewValue());
			Command command = service.handleMoveAttributeOperation((Clabject)removeNotification.getNotifier(), (Clabject)addNotification.getNotifier(), (Feature)addNotification.getNewValue());
			emendedObjects.clear();
		...
		for (Notification notification : event.getNotifications())
			...
			{	
					...
					)
				{
					...
					//for a trait change
					if((emendedObjects.iterator().hasNext() && ! (emendedObjects.iterator().next() instanceof Feature))){
						emendedObjects.clear();
						break;
					}
					...
					Collection<Feature> affectedModelElements = analyzer.getAffectedModelElementsForChangeTraitFeatureOperation((Feature)notification.getNotifier(), notification.getOldStringValue(), (EStructuralFeature)notification.getFeature());
					if (affectedModelElements.size() > 0){
						Command command = service.handleChangeFeatureTraitOperation((Feature)notification.getNotifier(), (EStructuralFeature)notification.getFeature(), notification.getOldStringValue(), notification.getNewStringValue());
						emendedObjects.clear();
				...
				)
			{
				...
				IImpactAnalyzer analyzer = service.getImpactAnalyzer();
				Collection<Clabject> affectedModelElements = analyzer.getAffectedModelElementsForChangeTraitClabjectOperation((Clabject)notification.getNotifier(), notification.getOldStringValue(), (EStructuralFeature)notification.getFeature(), true);
				if (affectedModelElements.size() > 0){
					Command command = service.handleChangeClabjectTraitOpertation((Clabject)notification.getNotifier(), (EStructuralFeature)notification.getFeature(), notification.getOldStringValue(), notification.getNewStringValue());
					emendedObjects.clear();
					//In case command is null no objects will be emended
					if (command != null)
						emendedObjects.addAll(service.getEmendedObjects(command));
					...
					&& notification.getNewValue() instanceof Attribute)
			{
				...
				IImpactAnalyzer analyzer = service.getImpactAnalyzer();
				Collection<Clabject> affectedModelElements = analyzer.getAffectedModelElementsForAddFeatureOperation((Clabject)notification.getNotifier(), (Feature)notification.getNewValue());
				if (affectedModelElements.size() > 0){
					Command command = service.handleAddAttributeOperation((Clabject)notification.getNotifier(), (Feature)notification.getNewValue());
					//In case command is null no objects will be emended
					if (command != null)
						emendedObjects.addAll(service.getEmendedObjects(command));
					return command;
		...
		//*********************************************
			...
			if (notification.getFeature() instanceof EStructuralFeature
				...
				IImpactAnalyzer analyzer = service.getImpactAnalyzer();
				Collection<Feature> affectedModelElements = analyzer.getAffectedModelElementsForDeleteFeatureOperation((Feature)notification.getOldValue(), (Clabject)notification.getNotifier());
									
				if (affectedModelElements.size() > 0){
					Command command = service.handleRemoveAttributeOperation((Clabject)notification.getNotifier(), (Feature)notification.getOldValue());
					emendedObjects.clear();
					//In case command is null no objects will be emended
					if (command != null)
						emendedObjects.addAll(service.getEmendedObjects(command));
