 ...
 * fixed after a change are calculated by this service. This service is the
 * foundation for the implementation of all emendation service commands.
 * 
 * <b> The result of all search operations is not allowed to contain the origin
 * of the impact calculation. This will break the emendation service otherwise.
 * </b>
 *
 */
public class BasicImpactAnalyzer implements IImpactAnalyzer {
  ...
  public Set<EObject> getAffectedModelElementsForMoveFeatureOperation(Clabject sourceChangeOrigin,
    ...
    result.addAll(
        getAffectedModelElementsForDeleteFeatureOperation(movedFeature, sourceChangeOrigin));
  ...
  public Set<Clabject> getAffectedModelElementsForAddFeatureOperation(Clabject changeOrigin,
    ...
    return getAffectedModelElementsForAddFeatureOperation(changeOrigin, addedFeature,
        addedFeature.getName(), includeDuplicates, false, true, true);
  }
  ...
  /**
   * Recursive method to search for clabjects which need the new feature to be
   * added starting from the list of clabjects contained in clabjects. Moves
   * upwards in the classification hierarchy.
   * 
   * @param clabjects
   *          List of clabjects to start recursion from
   * @return clabjects that need the feature to be added
   */
  private Set<Clabject> searchLevelForAddFeatureTypeDirection(Set<Clabject> clabjects,
      boolean includeSuperTypesOnly) {
    Set<Clabject> result = new HashSet<Clabject>();
    Set<Clabject> directTypes = new HashSet<Clabject>();

    for (Clabject c : clabjects) {
      // If we add an attribute to an clabject only this attribute is affected on the
      // same level
   ...
   * Recursive method to search for clabjects which need the new feature to be
   * added starting from the list of clabjects contained in clabjects. Moves
   * downwards in the classification hierarchy.
   * 
   * @param clabjects
   *          List of clabjects to start recursion from
   * @return clabjects that need the feature to be added
   */
  Set<Clabject> searchLevelForAddFeatureInstanceDirection(Set<Clabject> clabjects,
      boolean includeSuperTypesOnly) {
    Set<Clabject> result = new HashSet<Clabject>();
    ...
    for (Clabject c : clabjects) {
      ...
      result.addAll(
          searchLevelForAddFeatureInstanceDirection(directInstances, includeSuperTypesOnly));
    }
  ...
  public Set<Feature> getAffectedModelElementsForDeleteFeatureOperation(Feature changeOrigin,
    ...
    // containingClabject.getAllFeatures() works here because this clabject does not
    // contain
    // the feature at this point of time anymore
    for (Feature f : changeOriginContainingClabject.getAllFeatures())
      if (match(changeOrigin, changeOriginContainingClabject, f,
          PLMPackage.eINSTANCE.getElement_Name(), changeOrigin.getName())) {
        overridden = true;
        break;
      }
    ...
    currentLevel.add(changeOrigin);
    result.addAll(searchLevelForRemoveFeatureTypeDirection(currentLevel,
        changeOriginContainingClabject, changeOrigin));
    result.addAll(currentLevel);
    result.addAll(searchLevelForRemoveFeatureInstanceDirection(result,
        changeOriginContainingClabject, changeOrigin));
  ...
  /**
   * Searches for all features that need to be removed after removing the feature
   * stored in changeOrigin. Searches the upwards in the classification hierarchy.
   * 
   * @param features
   *          List of features to start recursion from
   * @param changeOriginContaining
   *          Clabject which contains the impact analysis triggering feature
   * @param changeOrigin
   *          The feature for which the impact analysis was initially triggered
   * @return Returns a list with features that need to be deleted after deleting
   *         change origin
   */
  Set<Feature> searchLevelForRemoveFeatureTypeDirection(Set<Feature> features,
      Clabject changeOriginContaining, Feature changeOrigin) {
    Set<Feature> result = new HashSet<Feature>();
    ...
    for (Feature f : features) {
      ...
      for (Clabject c : allTypes)
        for (Feature containedFeature : c.getFeature())
          if (match(f, thisFeatureContainingClabject, containedFeature,
              PLMPackage.eINSTANCE.getElement_Name(), f.getName()))
            allTypesFeatures.add(containedFeature);

      result.addAll(allTypesFeatures);
      result.addAll(searchLevelForRemoveFeatureTypeDirection(allTypesFeatures,
          changeOriginContaining, changeOrigin));
    }
  ...
  /**
   * Searches for all features that need to be removed after removing the feature
   * stored in changeOrigin. Searches the downwards in the classification
   * hierarchy.
   * 
   * @param features
   *          List of features to start recursion from
   ...
   *         change origin
   */
  Set<Feature> searchLevelForRemoveFeatureInstanceDirection(Set<Feature> features,
      Clabject changeOriginContaining, Feature changeOrigin) {
    Set<Feature> result = new HashSet<Feature>();
    ...
    for (Feature f : features) {
      ...
      result.addAll(searchLevelForRemoveFeatureInstanceDirection(allInstancesFeatures,
          changeOriginContaining, changeOrigin));
    }
  ...
  public Set<Feature> getAffectedModelElementsForChangeTraitFeatureOperation(Feature changeOrigin,
    ...
    // the feature at this point of time anymore
    for (Feature f : changeOriginContainingClabject.getAllFeatures())
      if (f != changeOrigin && match(changeOrigin, changeOriginContainingClabject, f,
          PLMPackage.eINSTANCE.getElement_Name(), changeOrigin.getName())) {
        overridden = true;
        break;
      }
    ...
    Set<Feature> currentLevel = new HashSet<Feature>();
    currentLevel.add(changeOrigin);
    result.addAll(searchLevelForChangeFeatureTraitFeatureTypeDirection(currentLevel, changeOrigin,
        changedTrait, oldValue));
    result.addAll(currentLevel);
    result.addAll(searchLevelForChangeFeatureTraitFeatureInstanceDirection(result, changeOrigin,
        changedTrait, oldValue));
    result.removeAll(currentLevel);
  ...
  /**
   * Recursively searches for features to change after changing changeOrigin. The
   * search direction is upwards in the classification hierarchy relative from
   * features.
   * 
   * @param features
   *          The features to start the recursion from
   * @param changeOrigin
   *          The initially changed feature which triggered the impact analysis
   * @param changedTrait
   *          The trait that has been changed
   * @param oldValue
   *          The trait value before the change
   * @return All features that need to be changed after applying a change to
   *         changeOrigin in type direction
   */
  Set<Feature> searchLevelForChangeFeatureTraitFeatureTypeDirection(Set<Feature> features,
      Feature changeOrigin, EStructuralFeature changedTrait, String oldValue) {
    Set<Feature> result = new HashSet<Feature>();
    Set<Clabject> allTypes = new HashSet<Clabject>();
    Set<Feature> allTypesFeatures = new HashSet<Feature>();
    ...
    for (Feature f : features) {
      ...
      for (Clabject c : allTypes)
        for (Feature containedFeature : c.getFeature())
          if (match(changeOrigin, changeOrigin.getClabject(), containedFeature, changedTrait,
              oldValue))
            allTypesFeatures.add(containedFeature);

      result.addAll(allTypesFeatures);
      result.addAll(searchLevelForChangeFeatureTraitFeatureTypeDirection(allTypesFeatures,
          changeOrigin, changedTrait, oldValue));
    }
  ...
  /**
   * Recursively searches for features to change after changing changeOrigin. The
   * search direction is downwards in the classification hierarchy relative from
   * features.
   * 
   * @param features
   *          The features to start the recursion from
   ...
   *          The trait value before the change
   * @return All features that need to be changed after applying a change to
   *         changeOrigin in instance direction
   */
  Set<Feature> searchLevelForChangeFeatureTraitFeatureInstanceDirection(Set<Feature> features,
      Feature changeOrigin, EStructuralFeature changedTrait, String oldValue) {
    Set<Feature> result = new HashSet<Feature>();
    Set<Clabject> allInstances = new HashSet<Clabject>();
    Set<Feature> allInstancesFeatures = new HashSet<Feature>();
    ...
    for (Feature f : features) {
      ...
      result.addAll(allInstancesFeatures);
      result.addAll(searchLevelForChangeFeatureTraitFeatureInstanceDirection(allInstancesFeatures,
          changeOrigin, changedTrait, oldValue));
    }
  ...
  public Set<Clabject> getAffectedModelElementsForChangeTraitClabjectOperation(
    ...
    Set<Clabject> affectedTypes = searchLevelForClabjectTraitChangesTypeDirectionForShallowPotencyChange(
        changeOrigins, includeNonShallowInheritance);
    result.addAll(affectedTypes);
    result.addAll(searchLevelForClabjectTraitChangesInstanceDirectionForShallowPotencyChange(
        affectedTypes, includeNonShallowInheritance));
  ...
  /**
   * Searches for the clabjects that need to be changed after changing a clabject.
   * The search direction is upwards in the classification hierarchy relative from
   * baseClabjects.
   * 
   * @param baseClabjects
   *          Clabjects to start recursion from
   * @return A list with all clabjects that need to be changed in type direction
   */
  private Set<Clabject> searchLevelForClabjectTraitChangesTypeDirectionForShallowPotencyChange(
      Set<Clabject> baseClabjects, boolean includeNonShallowInheritance) {
    Set<Clabject> result = new HashSet<Clabject>();
    ...
    for (Clabject changeOrigin : baseClabjects) {
      ...
      affectedClabjectsSameLevel.addAll(includeNonShallowInheritance ? changeOrigin.getSupertypes()
          : changeOrigin.getHollowSupertypes());
      ...
      while (iter.hasNext())
        affectedClabjectsSameLevelTmp
            .addAll(includeNonShallowInheritance ? iter.next().getSubtypes()
                : iter.next().getHollowSubtypes());

      affectedClabjectsSameLevel.addAll(affectedClabjectsSameLevelTmp);

      result.addAll(affectedClabjectsSameLevel);

      // Now we need all direct types of the model elements that are affected on one
      // level
      // Direct types are important as only these and their shallow supertypes are
      // affected
      ...
      result.addAll(searchLevelForClabjectTraitChangesTypeDirectionForShallowPotencyChange(
          affectedDirectTypes, includeNonShallowInheritance));
    }
  ...
  /**
   * Searches for the clabjects that need to be changed after changing a clabject.
   * The search direction is downwards in the classification hierarchy relative
   * from baseClabjects.
   * 
   * @param baseClabjects
   *          Clabjects to start recursion from
   * @return A list with all clabjects that need to be changed in instance
   *         direction
   */
  private Set<Clabject> searchLevelForClabjectTraitChangesInstanceDirectionForShallowPotencyChange(
      Set<Clabject> baseClabjects, boolean includeNonShallowInheritance) {
    Set<Clabject> result = new HashSet<Clabject>();
    ...
    for (Clabject changeOrigin : baseClabjects) {
      ...
      // Now we need all direct instances of the model elements that are affected on
      // one level
      // Direct instances are important as only these and their shallow supertypes are
      // affected
      // and not the none shallow ones
      ...
      result.addAll(searchLevelForClabjectTraitChangesInstanceDirectionForShallowPotencyChange(
          affectedDirectInstances, includeNonShallowInheritance));
    }
  ...
  /**
   * 
   * This checks whether two features are relevant for emendation. The conditions
   * are:
   * 
   * <ul>
   * <li>Name and Datatype (if Attribute) must be equal</li>
   * <li>Potency rules are not allowed to be violated
   * <ul>
   ...
   * <li>f1.potency > -1 && f1.level < f2.level => f2.potency = f1.potency -
   * (f2.level - f1.level)</li>
   * <li>f1.potency > -1 && f1.level > f2.level => f1.potency = f2.potency -
   * (f1.level - f2.level)</li>
   * </ul>
   * </li>
   * </ul>
   * 
   ...
   */
  private boolean match(Feature changeOrigin, Clabject originContainingClabject, Feature f2,
      EStructuralFeature traitToChange, String oldValue) {
    ...
    // null
    oldValue = traitToChange.getName().equals("durability") && oldValue == null
        ? changeOrigin.getDurabilityAsString()
        : oldValue;

    int originDurability = traitToChange.getName().equals("durability") ? Integer.parseInt(oldValue)
        : changeOrigin.getDurability();
    ...
    if (changeOrigin instanceof Attribute && f2 instanceof Attribute) {
      String originDataType = changedTraitName.equals("datatype") ? oldValue
          : ((Attribute) changeOrigin).getDatatype();
