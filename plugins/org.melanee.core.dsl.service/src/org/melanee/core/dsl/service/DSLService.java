...
public class DSLService implements IDSLService {
  // returns all connections between source and target assuming that source is a
  // supertype of target (if there is an inheritance relationship between the two
  // entities)
  private List<Connection> getInstantiableConnectionsBetween(Entity sourceInstance,
      List<Clabject> sourceTypes, List<Clabject> targetTypes) throws ParserException {
    List<Connection> result = new LinkedList<Connection>();

    // Also subtypes need to be taken into account maybe it is connected to
    // something that is subtype of the destination
    EList<Connection> sourceConnections = new BasicEList<Connection>((HashSet<Connection>) OCLHelper
        .execute(sourceInstance, "self.getLevelTypes().getConnections()->asSet()"));
    ...
    for (Connection obj : sourceConnections) {
      ...
      EList<EObject> participants = new BasicEList<EObject>((List<EObject>) OCLHelper.execute(obj,
          "self.connectionEnd.destination->union(self.connectionEnd.destination.getSubtypes())->asSequence()"));
      ...
      for (EObject participant : participants)
        ...
        if (targetTypes.contains(participant)) {
          ...
          EList<Connection> subConnections = new BasicEList<Connection>(
              (HashSet<Connection>) OCLHelper.execute(obj, "self.getSubtypes()"));
  ...
  @Override
  public List<Connection> getInstantiableConnectionsBetween(Entity sourceInstance,
    ...
    try {
      ...
      List<Connection> other = getInstantiableConnectionsBetween(targetInstance, targetTypes,
          sourceTypes);

      // add connections found by traversing the inheritance hierarchy from target
      // (instead of source) if they are not in 'result' already
  ...
  @Override
  public List<Connection> getInstantiableConnectionsBetween(Connection sourceInstance,
  ...
  public List<ConnectionEnd> getInstantiableConnectionEndsBetween(Connection sourceInstance,
    ...
    try {
      sourceConnectionTypes = (EList<Connection>) EMFReflectionHelper
          .executeOperation(sourceInstance, "getLevelTypes");
      targetTypes = (EList<EObject>) EMFReflectionHelper.executeOperation(targetInstance,
          "getLevelTypes");
