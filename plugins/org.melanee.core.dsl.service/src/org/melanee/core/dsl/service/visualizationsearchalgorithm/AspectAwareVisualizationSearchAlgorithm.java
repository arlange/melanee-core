...
public class AspectAwareVisualizationSearchAlgorithm {
  ...
  public AspectAwareVisualizationSearchAlgorithm(
      IAspectAwareVisualizationSearchAlgorithmHelper helper) {
  ...
  /**
   * The copier which can be used to resolve a model element to its original
   * visualizer
   */
  private EcoreUtil.Copier copier;

  /**
   * Merged visualizers return a copy only. When interpreting a visualizer is can
   * be important to know where the copied visualization descriptor currently
   * interpreted comes from. This operation returns the original visualization
   * descriptor for a given visualization descriptor
   * 
   * @param descriptor
   *          the descriptor for which the original descriptor is searched
   * @return the original visualization descriptor if one exists, descriptor
   *         otherwise
   */
  public EObject findOriginalVisualizationDescriptorForMergedVisualizationDescriptor(
      EObject descriptor) {
   ...
   *             findOriginalVisualizationDescriptorForMergedVisualizationDescriptor
  ...
  /**
   * Retrieves the parent for an visualizer which was merged through aspects.
   * 
   * @param visualizer
   *          the visualizer for which to retrieve the parent
   ...
   * IVisualizationServiceBase.NOTATION_LML is returned. This is also returned for
   * Domain and Enumeration as these do not have visualizers attached and do thus
   * not have any notation except the LML in which they are rendered. The notation
   * search starts searching at the element for a notation. If the notation of the
   * element to be searched is specified as
   ...
   * 
   * @param e
   *          The element to search the notation for.
   * @param format
   *          The format for which the notation is searched
   * @return The notation or IVisualizationServiceBase.NOTATION_LML if no notation
   *         is found
   */
  public static String searchNotationForElement(Element e, String format) {
    ...
    while (container.eContainer() != null && !(container.eContainer() == null)) {
  ...
  private AbstractDSLVisualizer search(Clabject e, boolean mergeAspects) {
    ...
    search: while ((currentType = typesToSearch.poll()) != null) {
      ...
      while ((currentClabject = superTypesToSearch.poll()) != null) {
        AbstractDSLVisualizer dslVisualizer = getVisualizerFromElement(currentClabject,
            elementToVisualize);
  ...
  /**
   * Searches a clabject for a dsl Visualizer. The dsl visualizer has to have the
   * correct notation, format and conform to the isSameLevel attribute.
   * 
   * @param currentClabject
   *          The clabject to search the dsl visualizer for
   * @param elementToVisualize
   *          The element to visualize. This is needed to check if the element to
   *          visualize and the visualizer are at the same level.
   * @return Returns a visualizer or null if none is found.
   */
  private AbstractDSLVisualizer getVisualizerFromElement(Element currentElement,
      Element elementToVisualize) {
    ...
    for (LMLVisualizer lmlVisualizer : currentElement.getVisualizer())
      ...
      for (AbstractDSLVisualizer dslVisualizer : lmlVisualizer.getDslVisualizer())
        ...
        if (helper.isRightFormat(dslVisualizer) && helper.isRightNotation(dslVisualizer)) {

          // Check if the visualizer is to be applied in case
          // the visualizer and visualized model element are
          // on the same level
          if (dslVisualizer.isInstanceLevel() && sameLevel(elementToVisualize, dslVisualizer))
            continue;
  ...
  /**
   * Checks if e and v are on the same leve. Returns false if e is not a clabject
   * 
   * @param e
   *          element to check
   * @param v
  ...
  /**
   * Searches and returns the aspect. If no aspect is found an empty list is
   * returned
   * 
   * @return
   */
  ...
  /**
   * Filters only the active aspects from all aspects. An aspect is active if the
  ...
  private List<EObject> getActiveAspects(AbstractDSLVisualizer v, Element visualizedElement) {
    ...
    for (EObject a : allAspects) {
      ...
      if (constraintLanguageService != null && visualizedElement instanceof Clabject)
        try {
          Object conditionResult = constraintLanguageService.evaluate(
              helper.getAspectContainingClabject(a), (Clabject) visualizedElement,
              helper.getCondition(a));
  ...
  private AbstractDSLVisualizer mergeVisualizer(AbstractDSLVisualizer v,
      Map<String, EObject> name2aspect) {
