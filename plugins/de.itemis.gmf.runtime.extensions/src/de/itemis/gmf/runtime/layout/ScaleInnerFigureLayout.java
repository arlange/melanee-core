...
public class ScaleInnerFigureLayout extends ConstrainedToolbarLayout implements LayoutManager {
  ...
  private boolean stretchMinor;

  public ScaleInnerFigureLayout() {
    // changed for visualizers
  ...
  public void setStretchMinor(boolean stretchMinor) {
  ...
  public void layout(IFigure parent) {
    ...
    int wHint = (isStretchWidth()) ? parent.getClientArea(Rectangle.SINGLETON).width : -1;
    int hHint = (isStretchHeight()) ? parent.getClientArea(Rectangle.SINGLETON).height : -1;
    ...
    for (int i = 0; i < numChildren; i++) {
      ...
      if (isStretchHeight()) {
        if (amntShrinkHeight > 0 && prefMinSumHeight != 0)
          amntShrinkCurrentHeight = (prefHeight - minHeight) * amntShrinkHeight
              / (prefMinSumHeight);
  ...
  protected boolean isStretchHeight() {
    return (isHorizontal() && getStretchMinorAxis()) || getStretchMajorAxis();
  }

  protected boolean isStretchWidth() {
    return (isHorizontal() && getStretchMajorAxis()) || getStretchMinorAxis();
  }
