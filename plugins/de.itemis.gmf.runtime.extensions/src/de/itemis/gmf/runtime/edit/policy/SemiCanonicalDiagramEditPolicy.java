 ...
 * @author koehnlein
 */
public abstract class SemiCanonicalDiagramEditPolicy extends CanonicalConnectionEditPolicy {
  ...
  public void addConnections(Collection<EObject> connectableSemanticElements) {
    ...
    removeExistingAndIrrelevantLinks(allLinks, connectableSemanticElements, diagram);
   ...
   */
  @SuppressWarnings("unchecked")
  protected abstract Collection collectAllLinks(View view, Map domain2NotationMap);
  ...
  /*
   * Methods to be implemented in generated code as they refer to the statically
   * typed classes XXXLinkDescriptor and XXXVisualIDRegistry.
   */
  ...
  protected abstract boolean isDescriptorMatchesEdge(Object linkDescriptor, Edge edge);
  ...
  private Collection removeExistingAndIrrelevantLinks(Collection allLinks,
    ...
    ALL_LINKS: for (Iterator iterator = allLinks.iterator(); iterator.hasNext();) {
      Object linkDescriptor = iterator.next();
      if (!isLinkSourceInList(linkDescriptor, selectedSemanticElements)
          && !isLinkTargetInList(linkDescriptor, selectedSemanticElements)) {
        iterator.remove();
        continue ALL_LINKS;
      }

      for (Object existingConnection : existingConnections) {
        if (isDescriptorMatchesEdge(linkDescriptor, (Edge) existingConnection)) {
          iterator.remove();
          continue ALL_LINKS;
        }
      }
