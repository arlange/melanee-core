...
public class ConstraintHookRegistry {
  ...
  public static ArrayList<ILinkConstraintDefinition> getLinkConstraintDefinitions(
      EObject container) {
    ArrayList<ILinkConstraintDefinition> result = new ArrayList<ILinkConstraintDefinition>();
    IConfigurationElement[] configurationElements = Platform.getExtensionRegistry()
        .getConfigurationElementsFor("de.itemis.gmf.runtime.extensions.linkConstraintHook");
    for (IConfigurationElement configurationElement : configurationElements) {
      ...
      if (container.eClass().getEPackage().getNsURI().equals(definedNsURI)) {
        try {
          result.add(
              (ILinkConstraintDefinition) configurationElement.createExecutableExtension("class"));
  ...
  public static List<INodeConstraintDefinition> getNodeConstraintDefinitions(EObject container) {
    ...
    for (IConfigurationElement configurationElement : configurationElements) {
      ...
      if (container.eClass().getEPackage().getNsURI().equals(definedNsURI)) {
        try {
          result.add(
              (INodeConstraintDefinition) configurationElement.createExecutableExtension("class"));
        } catch (CoreException e) {
          e.printStackTrace();
        }
      }
  ...
  private static IConfigurationElement[] getConfigurationElements(String id) {
    return Platform.getExtensionRegistry()
        .getConfigurationElementsFor("de.itemis.gmf.runtime.extensions.nodeConstraintHook");
  }
}
